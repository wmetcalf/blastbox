...
[redeploy-warm] DONE (legacy-rebuild). ROLLBACK if needed:
  ...
  if [ -e $GVISOR_DIR/rootfs.$SUF ]; then sudo rm -rf $GVISOR_DIR/rootfs && sudo mv $GVISOR_DIR/rootfs.$SUF $GVISOR_DIR/rootfs; else echo "NO gVISOR BACKUP at $GVISOR_DIR/rootfs.$SUF - leaving the live tree in place"; fi
...
[redeploy-warm] DONE (recreate). ROLLBACK if needed:
  ...
  if [ -e $GVISOR_DIR/rootfs.bak ]; then sudo rm -rf $GVISOR_DIR/rootfs && sudo mv $GVISOR_DIR/rootfs.bak $GVISOR_DIR/rootfs; else echo "NO gVISOR BACKUP at $GVISOR_DIR/rootfs.bak - leaving the live tree in place"; fi
  [ -f $FC_DIR/firecracker.$SUF ] && mv $FC_DIR/firecracker.$SUF $FC_DIR/firecracker
  $COMPOSE_WRAPPER $COMPOSE_FILES up -d --force-recreate api dispatcher dispatcher-fc dispatcher-gvisor
ROLLBACK
fi
