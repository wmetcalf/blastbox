...
| Var | Default | Notes |
|---|---|---|
| `BLASTBOX_DATABASE_URL` (on a **node**) | — | an `https://` control-plane URL here instead of a DSN gives the node a store that claims through the authenticated hand-over and holds **no database credentials** (#178). Needs `BLASTBOX_NODE_CERT` (the key is the sibling `.key`) and the CA's public half. Such a node cannot submit, delete or enumerate — those raise. Same variable on purpose: a node talks to the database or the control plane, never both |
...
| `BLASTBOX_API_KEY` | `""` (open) | Optional bearer token required on the API. Usually unset — auth is delegated to a reverse proxy.. **The node-claim routes (`/v1/nodes/…`) are exempt**: they authenticate with a CA-issued certificate and a session bound to it, and requiring the key too would put the submitter's credential on every node |
...
| `BLASTBOX_MAX_QUEUED_AGE_S` | `0` (off) | Opt-in **stale-queued reaper**: TTL after which a job still QUEUED is FAILed and its (untrusted) input deleted — bounds the `target_tier` footgun (a job pinned to a tier no dispatcher serves) and a >1k batch backlog. Honored by **every** dispatcher variant: the cold container `Dispatcher` and the network-endpoint `VmJobDispatcher`, **and by `blastbox serve`** — on the ingress host it also FAILs jobs nobody claimed and deletes their untrusted input, which is how `target_tier`-pinned work is bounded on an all-federated fleet (libvirt-VM / static / AWS / cascade). `0` ⇒ never reap on age (correct for huge legitimate batches). **On a federated fleet it also disposes of work no enrolled node can run**: the control plane excludes such a job from every node the first time it is refused (logging, once, which grant is missing), so it stops being churned and sits QUEUED for this sweep to fail. Without it that work stays queued harmlessly until a node holding the grant enrols — the exclusion lifts the moment the certificate set changes. |
...
| `BLASTBOX_LIBVIRT_URI` | `""` | libvirt connection URI passed to every `virsh` call as `-c` (e.g. `qemu:///system`, `qemu:///session`, `qemu+ssh://root@host/system`). Unset = whatever `virsh` defaults to for the invoking user, which is a property of *who is running*, not of the host. Needed on any box running more than one libvirt: a per-worker `connect_uri` in `vmcompose.yml` overrides this. Deliberately not `LIBVIRT_DEFAULT_URI`, which everything else on the host would inherit. |
| `BLASTBOX_APPARMOR_PROFILE` | `blastbox-sandbox` | Name of the AppArmor profile attached to the **detonated child** (`aa-exec -p <name>`), for both the `nsjail` and `bwrap` backends. This repo **ships** a profile under the default name (`deploy/apparmor/blastbox-sandbox`); it still has to be loaded on the host (`apparmor_parser -r -W`), and until it is, both namespace backends report `apparmor_missing`. See `deploy/apparmor/README.md` before adapting it — attaching a profile is also what makes nsjail pass `--proc_rw`. |
| `BLASTBOX_APPARMOR_OBSERVED` | — | Set **by the dispatcher**, not by hand: `<profile>:<mode>` (comma-separated for several), the mode the kernel reported on the host when that container was launched. `absent` is a mode too — the dispatcher read securityfs and the profile was not in it, which is a stronger disproof than `complain`. Not set on the gVisor warm/snapshot tier, whose environment is frozen at checkpoint time and could only carry a stale claim. Outranks `BLASTBOX_APPARMOR_PROFILES`, because it carries the MODE — so a profile the host sees in `complain` becomes a positive disproof in the worker instead of an unverifiable claim. The worker's own securityfs read, where possible, still outranks both. |
...
| `BLASTBOX_ALLOW_CONFINEMENT_LOSS` | `0` | Lets a worker keep running after the AppArmor profile it was **admitted with** stops enforcing (unloaded, switched to complain, or no longer confirmable). Deliberately NOT covered by `BLASTBOX_WARN_ON_INSECURE`: the dispatcher sets that one automatically for every runsc worker, so honouring it here would switch this refusal off everywhere the fleet runs. A backend that never had a profile is unaffected either way. |
...
| `BLASTBOX_SNAPSHOT_MAX_AGE_S` | `21600` (6h) | Maximum age of a warm snapshot BASE before it is refreshed. **Shared by the Firecracker and gVisor tiers.** A base restored long after its checkpoint was taken eventually hangs every job at the worker timeout (observed at roughly a day), so an aged base builds its replacement in the background while it KEEPS SERVING, and is swapped out only once the replacement is ready — a failed refresh backs off and leaves the current base in place. The swap takes effect at the pool's next tick, so each slot's generation stamp matches the base it restored from. A HARD CEILING of 2× this age bounds a refresh that keeps failing: past it, the base is dropped (the pool falls back to its other tiers) rather than serving restores expected to hang, and idle slots whose checkpoint is that old are retired. A refresh needs room for a second generation in the snapshot mem dir until the swap. `0` disables; must be finite and ≥ 0 (anything else falls back to the default). |
...
| `BLASTBOX_GVISOR_ROOTLESS` | off | Drive `runsc` without host root (`-rootless`). **Refused by the warm-snapshot tier**: gVisor does not support `restore` rootless (`Rootless mode not supported with "restore"`), so that tier fails at construction rather than on its first job. **No current tier can run rootless**: the snapshot tier is the only one that reads this, and the docker `--runtime=runsc` worker path ignores it — so today setting it only produces that refusal. The measured groundwork for a future non-snapshot tier: it pins the worker to **guest uid 0** — gVisor's gofer runs as the calling user and cannot create bind-mount files as an unmapped uid (the failure is `EINVAL` on `/ctrl/.ready.tmp`). The trade: rootful keeps the in-sandbox uid drop but runs runsc as host root, so an escape lands as root; rootless gives up the drop, but guest uid 0 is the unprivileged launching user. On Ubuntu 24.04+ it also needs `deploy/apparmor/blastbox-runsc` loaded. |
...
| `BLASTBOX_PKI_DIR` (on the **ingress** host) | `/var/lib/blastbox/pki` | if a `ca.crt` is here, ingress also serves the seven `/v1/nodes/…` routes (challenge, session, claim, backlog, job read, job report, disposition — see DEPLOYMENT.md, and allowlist all of them), which check a node's certificate and grants **before** handing over a job (#178). Nothing to switch on; no CA means the routes do not exist. A node holding `BLASTBOX_DATABASE_URL` can still claim from the store directly, so for such a node this is defence in depth, not a gate — see DEPLOYMENT.md |
| `BLASTBOX_NODE_CA` | `<BLASTBOX_PKI_DIR>/ca.crt` | CA the node verifies the **control plane's** TLS certificate against. Note it does not move the *local* grants gate, which reads `BLASTBOX_PKI_DIR/ca.crt` — put the CA in the PKI directory unless you mean the two to differ |
| `BLASTBOX_NODE_CLAIM_STRICT_TIERS` (on the **ingress** host) | — (off) | refuse a claim whose network personality this host cannot resolve, instead of handing it over unchecked. Off by default because an ingress with no `BLASTBOX_NETPOLICY_*` registry would otherwise refuse every ordinary job; ON is what makes the tier and credentials grants enforced in a deployment whose engine defaults live only on the dispatchers |
| `BLASTBOX_NODE_KEY` (on a **node**) | the `.key` beside `BLASTBOX_NODE_CERT` | override for a split-mount layout where the node's private key is not next to its certificate |
| `BLASTBOX_TLS_CERT` / `BLASTBOX_TLS_KEY` (on the **ingress** host) | — | env equivalents of `serve --tls-cert/--tls-key`; an explicit pair always wins over auto-issuance |
| `BLASTBOX_MAINTENANCE_INTERVAL_S` | `60` | how often the dispatcher **and** `blastbox serve` run their maintenance sweeps. `0` disables the loop entirely — on a credential-less fleet that removes the ONLY reclaim path for lost claims |
| `BLASTBOX_TLS_SANS` (on the **ingress** host) | — | comma-separated extra names for the auto-issued TLS certificate. With `--host 0.0.0.0` the machine's own hostname and addresses are included automatically; add the name nodes are actually configured with (a load balancer's DNS name) here, or nodes will refuse the certificate |
| `BLASTBOX_JOB_RETENTION_SECONDS` (on the **ingress** host) | `0` | also runs the job retention sweep on the control plane, with the ingress blob store, for fleets whose nodes claim through it (they cannot enumerate the queue to expire). Same variable as the dispatcher's |
| `BLASTBOX_NODE_CLAIM_RECLAIM_AFTER_S` (on the **ingress** host) | — (sweep off) | **Required for a credential-less fleet.** Fail RUNNING jobs idle longer than this many seconds (floored at 900). Nodes claiming through the control plane cannot run the dispatcher's orphan sweep, so without this every lost claim stays RUNNING forever. Abandoned jobs are failed, not requeued. Ingress warns at startup if unset |
| `BLASTBOX_CLAIM_SECRET_FILE` | — | **Override only.** The key that signs node challenges, sessions and receipts is recorded on the **job queue** (every shipped store supports this), so ingress hosts agree with nothing configured. Set this to hold the key in a file instead — for a third-party store without the registry, or a deliberate reason. When set, it wins. A PATH, never the secret itself. Inspect/rotate with `blastbox claim-key show|reset` |
