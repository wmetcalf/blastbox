...
| `BLASTBOX_SCRATCH_MAX_AGE_S` | `21600` (6h) | Age after which a per-job scratch dir under `JOB_ROOT` is reclaimed by dispatcher maintenance (both the container and the network-tier dispatchers). **Local scratch only — never touches the blob store.** It bounds the trees the terminal purge deliberately keeps: one whose worker container was never confirmed dead, and one whose result upload exhausted its retries (then the only copy). A tree is reclaimed only when the newest mtime *anywhere* beneath it is older than this, its job is terminal or unknown to the store, its worker container is not one this dispatcher is still waiting to confirm dead, and — if it holds a sealed result — that result is positively present in the blob store, so a long-running job is never reclaimed and a result with no durable copy (including a pre-blob-store legacy job) is never the last thing deleted. A result whose upload failed is not stranded by that hold: dispatcher maintenance retries the upload on every tick, and the tree is reclaimed normally once the durable copy lands — but if you run with `BLASTBOX_WORKER_TIMEOUT_S` above this value, raise it to match. `0` disables reclaim entirely (trees then persist until you sweep them yourself). |
