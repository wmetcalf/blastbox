...
| `BLASTBOX_CLAIM_SECRET_FILE` | `<pki>/claim-challenge.key` | where the key that signs node challenges and session tokens lives. **Every ingress host serving one address must point at the same file** — forked workers on one host already share it, but two hosts each generate their own, and behind a load balancer a node's handshake then fails whenever the challenge and the session land on different hosts. A PATH, never the secret itself |
