...
def test_seal_within_cap_uses_stat_size(tmp_path):
    ...
    assert (
        env.artifacts[0].bytes == 5
    )  # under cap -> sealed; size from stat, chunk-hashed
...
def test_confined_atomic_writer_blocks_symlinked_parent(tmp_path):
    ...
    assert not (
        outside_dir / "x.png"
    ).exists()  # write never escaped into the symlink target
...
def test_confined_atomic_writer_dir_mode_for_cross_uid_traverse(tmp_path):
    ...
    with confined_atomic_writer(
        d, "nested/sub/art.txt", mode=0o644, dir_mode=0o755
    ) as fd:
        os.write(fd, b"png")
...
def test_atomic_write_confined_defeats_temp_symlink(tmp_path):
    ...
    outside = tmp_path / "outside"
    outside.write_bytes(b"ORIGINAL")
    (d / ".metadata.json.tmp").symlink_to(
        outside
    )  # the previously-predictable temp path
...
def test_seal_allows_nested_metadata_json_artifact(tmp_path):
    ...
    env = seal_envelope(
        ...
        detected=_det(),
        declared=[
            DeclaredArtifact(id="rmeta", path="rmeta/metadata.json", kind="json")
        ],
        warnings=[],
        payload=ExtractedText(text="x", char_count=1),
    )
