...
def test_serve_rejects_partial_tls():
    ...
    with pytest.raises(SystemExit, match="set together"):
        serve(
            _NoopEngine(), bind="0.0.0.0", port=0, tls_cert="/c.pem"
        )  # cert without key
...
def test_serve_clear_error_on_unreadable_tls_material(tmp_path):
    ...
    with pytest.raises(
        SystemExit, match="cannot load TLS material.*READABLE by the agent uid"
    ):
        serve(
            _NoopEngine(),
            bind="0.0.0.0",
...
def test_hard_deadline_retires_hung_engine(monkeypatch):
    ...
    release = (
        threading.Event()
    )  # the test releases the "hang" so _JOB_LOCK (module-global!) is freed
    ...
    class _Hang:
        ...
        def detonate(self, input, outdir, limits):
            ...
            raise RuntimeError(
                "released"
            )  # then error -> 500 -> do_POST frees _JOB_LOCK
    ...
    def _do_post():
        with contextlib.suppress(Exception):
            _post(
                f"http://127.0.0.1:{port}/detonate?name=x.bin", b"MZ"
            )  # hangs in the detonation
    ...
    try:
        assert fired.wait(
            3
        )  # the watchdog fired within the deadline (os._exit seam invoked)
    finally:
...
def test_serve_allows_wide_open_with_insecure_optin(monkeypatch):
    ...
    httpd = serve(
        _NoopEngine(), bind="0.0.0.0", port=0
    )  # explicit opt-in -> warns + serves
    httpd.server_close()
...
def test_guard_exposure_allows_gated_or_optin():
    ...
    _guard_exposure(
        "::1", 8765, token=None, client_ca=None, allow_cidrs=None, allow_insecure=False
    )
    _guard_exposure(
        "0.0.0.0",
        8765,
...
def test_slow_body_returns_408():
    ...
    try:
        ...
        s.sendall(
            b"POST /detonate?name=x.bin HTTP/1.1\r\nHost: x\r\nContent-Length: 1000000\r\n\r\n"
        )
        for _ in range(
            6
        ):  # trickle: byte every 0.3s, no idle gap > 0.5s, total > 0.5s budget
            try:
                s.sendall(b"x")
            except OSError:
...
def test_dir_bomb_trips_entry_cap(monkeypatch):
    ...
    monkeypatch.setenv(
        "BLASTBOX_MAX_ARTIFACTS", "3"
    )  # file_cap = 19 -> entry_cap = 140
    ...
    assert (
        exc is not None and exc.code == 500
    )  # entry cap tripped -> 500, agent stays up
...
def test_nested_artifact_layout_passes(monkeypatch):
    ...
    monkeypatch.setenv(
        "BLASTBOX_MAX_ARTIFACTS", "20"
    )  # file_cap = 36 -> entry_cap = 4*36+64 = 208
    ...
    class _Nested:
        ...
        def detonate(
            ...
            for i in range(20):  # 20 artifacts in per-page subdirs -> ~41 entries < 208
                ...
                arts.append(
                    DeclaredArtifact(
                        id=f"a{i}", path=f"page-{i:03d}/img.png", kind="image"
                    )
                )
            return DetonationResult(
                payload=Page(
                ...
                artifacts=arts,
                detected=Detection(
                    label="docx", mime="x", confidence=0.9, source="test"
                ),
            )

    assert (
        _serve_engine_and_post(_Nested()) is None
    )  # 200 -- valid nested output archived, not 500
...
class _EnvEchoEngine:
    ...
    def detonate(self, input: Path, outdir: Path, limits: Limits) -> DetonationResult:
        ...
        return DetonationResult(
            ...
            detected=Detection(
                label=os.environ.get("TEST_PARAM", "<unset>"),
                mime="application/octet-stream",
                confidence=1.0,
                source="test",
            ),
...
class _EgressEchoEngine:
    ...
    def detonate(self, input: Path, outdir: Path, limits: Limits) -> DetonationResult:
        ...
        return DetonationResult(
            ...
            artifacts=[DeclaredArtifact(id="a0", path="p.png", kind="image")],
            detected=Detection(
                label=("egress" if limits.net_egress else "sealed"),
                mime="application/octet-stream",
                confidence=1.0,
                source="test",
            ),
...
def test_forwarded_net_egress_reaches_limits(tmp_path):
    ...
    try:
        ...
        assert (
            meta["detected"]["label"] == "egress"
        )  # forwarded param took effect on Limits
        meta2 = detonate_remote(f"http://127.0.0.1:{port}", inp, tmp_path / "o2")
...
def test_input_named_out_does_not_collide_with_output_dir():
    ...
    try:
        ...
        assert (
            r.status == 200
        )  # a client filename of "out" no longer clashes with out_dir
    finally:
        httpd.shutdown()
        httpd.server_close()
...
def test_healthz_requires_token_when_set():
    ...
    try:
        ...
        req = urllib.request.Request(
            f"http://127.0.0.1:{port}/healthz", headers={"X-aws-proxy-auth": "s3cr3t"}
        )
...
def test_zero_byte_input_accepted():
    ...
    try:
        ...
        assert (
            r.status == 200
        )  # a zero-byte sample is valid (the local harness runs it too)
    finally:
        httpd.shutdown()
        httpd.server_close()
...
def test_agent_caps_oversized_metadata_before_archiving(monkeypatch):
    ...
    monkeypatch.setenv(
        "BLASTBOX_MAX_METADATA", "50"
    )  # any real sealed metadata.json exceeds 50 bytes
    httpd, port = _running_agent()
    try:
        with pytest.raises(urllib.error.HTTPError) as ei:
            _post(f"http://127.0.0.1:{port}/detonate?name=x.bin", b"data")
        assert (
            ei.value.code == 500
        )  # over the metadata cap -> job fails, not a huge disk write
    finally:
        httpd.shutdown()
        httpd.server_close()
...
def test_concurrent_detonate_returns_409_busy():
    ...
    try:
        ...
        th.join(timeout=10)
        assert (
            r1.get("r") is not None and r1["r"].status == 200
...
def test_healthz_honors_cidr_allowlist():
    ...
    try:
        ...
        assert (
            ei.value.code == 403
        )  # readiness gated same as /detonate -> no false "healthy"
    finally:
        httpd.shutdown()
        httpd.server_close()
...
def test_mtls_end_to_end_and_rejects_uncertified_caller(tmp_path):
    ...
    try:
        ...
        meta = detonate_remote(
            f"https://127.0.0.1:{port}", inp, tmp_path / "o", ssl_context=cctx
        )
        ...
        with pytest.raises(Exception):
            detonate_remote(
                f"https://127.0.0.1:{port}", inp, tmp_path / "o2", ssl_context=noclient
            )
    finally:
        httpd.shutdown()
        httpd.server_close()
