...
class TestArgv:
    ...
    def test_block_net_invariant_regardless_of_net_egress(self):
        ...
        for egress in (False, True):
            ...
            assert "--block-net" in argv, (
                f"nono must stay block-net (net_egress={egress})"
            )
    ...
    def test_grants_system_dirs_read_only(self):
        ...
        assert any(
            argv[i] == "-r" and argv[i + 1] == "/usr" for i in range(len(argv) - 1)
        )
        # /tmp is granted read+write (the child HOME)
        assert any(
            argv[i] == "-a" and argv[i + 1] == "/tmp" for i in range(len(argv) - 1)
        )
    ...
    def test_ro_mount_becomes_read_grant_rw_becomes_allow(self, tmp_path):
        ...
        assert any(
            argv[i] == "-r" and argv[i + 1] == str(ro) for i in range(len(argv) - 1)
        )
        assert any(
            argv[i] == "-a" and argv[i + 1] == str(rw) for i in range(len(argv) - 1)
        )
    ...
    def test_file_mounts_use_file_grants(self, tmp_path):
        ...
        argv = _sb()._build_argv(req)
        assert any(
            argv[i] == "--read-file" and argv[i + 1] == str(rofile)
            for i in range(len(argv) - 1)
        )
        assert any(
            argv[i] == "--allow-file" and argv[i + 1] == str(wfile)
            for i in range(len(argv) - 1)
        )
...
def test_run_relocates_state_and_returns_result(tmp_path):
    ...
    sb = NonoSandbox(
        nono_bin=_FAKE_NONO, state_dir=tmp_path / "state", popen=fake_popen
    )
