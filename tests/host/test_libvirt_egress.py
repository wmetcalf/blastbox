...
def test_apply_fails_closed_for_inetsim_without_fakenet_addr():
    ...
    with pytest.raises(ValueError):
        eg.apply(
            "192.168.122.5",
            VmEgressPolicy(exit_driver="inetsim"),
            "192.168.122.1",
...
def test_apply_rejects_unsupported_exit_driver():
    ...
    for drv in ("socks", "httpproxy", "bogus"):
        with pytest.raises(ValueError):
            eg.apply(
                "192.168.122.5",
                VmEgressPolicy(exit_driver=drv),
                "192.168.122.1",
                mac="52:54:00:aa:bb:cc",
            )
...
def test_apply_allows_complete_shared_router_config(monkeypatch):
    ...
    eg = LibvirtEgress(
        sudo=False, routing=ExitRouting(gateway="192.168.50.1", leg="br-vpn1")
    )
    with pytest.raises(Exception) as ei:
        eg.apply(
            "192.168.122.5",
            VmEgressPolicy(exit_driver="openvpn"),
            "192.168.122.1",
            mac="52:54:00:aa:bb:cc",
        )
    assert "gateway and leg" not in str(
        ei.value
    )  # passed the XOR gate; failed later on the rc1 op
...
def test_drop_policy_has_no_dns_exemption():
    ...
    rules = _flat(
        forward_chain_rules(
            "192.168.122.5", VmEgressPolicy(exit_driver="drop"), "192.168.122.1"
        )
    )
...
def test_web_only_block_internal():
    ...
    pol = VmEgressPolicy(
        exit_driver="direct", egress_ports=(53, 80, 443), block_internal=True
    )
...
def test_exit_drop_drops_everything():
    rules = forward_chain_rules(
        "10.0.0.5", VmEgressPolicy(exit_driver="drop"), gateway=None
    )
...
def test_exit_none_drops_everything():
    rules = forward_chain_rules(
        "10.0.0.6", VmEgressPolicy(exit_driver="none"), gateway=None
    )
...
def test_direct_no_ports_accepts():
    rules = forward_chain_rules(
        "192.168.122.9", VmEgressPolicy(exit_driver="direct"), gateway=None
    )
...
def test_from_personality_fails_closed_on_invalid_ports():
    ...
    p = Personality(
        name="w", exit_driver="direct", config={"egress_ports": "70000 nope"}
    )
    pol = VmEgressPolicy.from_personality(p)
...
def test_from_personality_block_internal_valid_values():
    assert (
        VmEgressPolicy.from_personality(
            Personality(
                name="w", exit_driver="direct", config={"block_internal": "true"}
            )
        ).block_internal
        is True
    )
    assert (
        VmEgressPolicy.from_personality(
            Personality(
                name="w", exit_driver="direct", config={"block_internal": "false"}
            )
        ).block_internal
        is False
    )
    assert (
        VmEgressPolicy.from_personality(
            Personality(name="w", exit_driver="direct", config={})
        ).block_internal
        is False
    )  # omitted → off
...
def test_routing_vpn_policy_routes_with_local_exemption():
    cmds = _flat(
        routing_commands(
            "192.168.122.5", "openvpn", ExitRouting(vpn_table="vpn", vpn_tun="tun0")
        )
    )
    # local subnet stays on main, everything else -> vpn table, MASQUERADE on the tun
    assert any(
        "rule add from 192.168.122.5 to 192.168.122.0/24 lookup main" in c for c in cmds
    )
...
def test_routing_tor_redirects_tcp_and_dns_keeps_local():
    cmds = _flat(
        routing_commands(
            "192.168.122.7", "tor", ExitRouting(tor_trans_port=9040, tor_dns_port=9053)
        )
    )
    ...
    assert cmds.index(next(c for c in cmds if "--dport 53" in c)) < cmds.index(
        next(c for c in cmds if "RETURN" in c)
    )


def test_routing_fakenet_dnats_when_addr_set():
    assert (
        routing_commands("192.168.122.9", "inetsim", ExitRouting()) == []
    )  # no addr -> disabled
    ...
    cmds = _flat(
        routing_commands(
            "192.168.122.9", "inetsim", ExitRouting(fakenet_addr="172.28.100.2")
        )
    )
...
def test_routing_fakenet_exempts_only_gateway_not_subnet():
    ...
    cmds = _flat(
        routing_commands(
            "192.168.122.9",
            "inetsim",
            ExitRouting(fakenet_addr="172.28.100.2"),
...
def test_tor_all_tcp_without_egress_ports():
    ...
    cmds = _flat(
        routing_commands("192.168.122.7", "tor", ExitRouting(tor_trans_port=9040))
    )
    assert any(
        c.endswith("-p tcp -j REDIRECT --to-ports 9040") for c in cmds
    )  # catch-all
...
def test_tor_port_scoped_with_egress_ports():
    ...
    assert any(
        "--dport 53 -j REDIRECT --to-ports 9053" in c for c in cmds
    )  # DNS -> DNSPort
...
def test_tor_forward_blocks_non_tcp_regardless_of_ports():
    ...
    for ep in (None, (80, 443)):
        fwd = _flat(
            forward_chain_rules(
                "192.168.122.5",
                VmEgressPolicy(exit_driver="tor", egress_ports=ep),
                "192.168.122.1",
            )
        )
        assert fwd[-1].endswith("-j DROP")
        assert not any(
            "multiport" in r for r in fwd
        )  # tor port-scoping is in nat, not FORWARD
        assert not any(
            r.endswith("-p tcp -j ACCEPT") for r in fwd
        )  # no blanket TCP accept in FORWARD
...
def test_vpn_killswitch_scopes_egress_to_tunnel():
    ...
    assert any(
        r.endswith("-o tun0 -j ACCEPT") for r in fwd
    )  # egress only via the tunnel
    ...
    assert (
        fwd[0]
        == "-A BBVM_192_168_122_5 -o tun0 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT"
    )
    # with an allow-list, even the allowlisted ports are tunnel-scoped
    fwd2 = _flat(
        forward_chain_rules(
            "192.168.122.5",
            VmEgressPolicy(exit_driver="openvpn", egress_ports=(80, 443)),
            "192.168.122.1",
            egress_if="tun0",
        )
    )
...
def test_routing_tor_dns_redirect_gated_on_allowlist():
    ...
    flat = [
        " ".join(c)
        for c in routing_commands("192.168.122.5", "tor", rt, egress_ports=(80, 443))
    ]
    assert not any("--dport 53 -j REDIRECT" in f for f in flat)
    # 53 present => DNS resolves via tor's DNSPort
    flat2 = [
        " ".join(c)
        for c in routing_commands("192.168.122.5", "tor", rt, egress_ports=(53, 80))
    ]
...
def test_input_chain_tor_dnsport_gated_on_allowlist():
    ...
    flat = _flat(
        input_chain_rules(
            "192.168.122.9",
            VmEgressPolicy(exit_driver="tor", egress_ports=(80, 443)),
            gateway="192.168.122.1",
            routing=rt,
        )
    )
...
def test_nat_chain_deletes_sweeps_return_bypass_rules():
    ...
    dels = _nat_chain_deletes(
        "192.168.122.50", "PREROUTING", ("REDIRECT", "DNAT", "RETURN"), pre
    )
    assert (
        sum("-j RETURN" in " ".join(d) for d in dels) == 2
    )  # both .50 RETURNs swept, not .5's
    assert all("192.168.122.50/32" in " ".join(d) for d in dels)
...
def test_v6_drop_raises_on_permission_denied(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda argv, **k: subprocess.CompletedProcess(
            ...
            4,
            "",
            "ip6tables: can't initialize ip6tables table `filter': Permission denied",
        ),
    )
...
def test_v6_drop_raises_on_missing_mac_match_extension(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda argv, **k: subprocess.CompletedProcess(
            ...
            1,
            "",
            "ip6tables: Couldn't load match `mac':No such file or directory",
        ),
    )
    with pytest.raises(RuntimeError, match="v6 fail-closed"):
        eg._v6_drop("FORWARD", "52:54:00:aa:bb:cc")
...
def test_apply_installs_routing_before_forward_jump(monkeypatch):
    ...
    dnat = next(
        i for i, s in enumerate(issued) if "DNAT --to-destination 172.28.100.1" in s
    )
    jump = next(
        i
        for i, s in enumerate(issued)
        if "FORWARD 1 -s 192.168.122.5 -j BBVM_192_168_122_5" in s
    )
    assert (
        dnat < jump
    )  # sink DNAT installed before the FORWARD jump → no clearnet window
...
def test_apply_exempts_dhcp_above_input_antispoof(monkeypatch):
    ...
    monkeypatch.setattr(eg, "_ipt_run", fake_ipt)
    monkeypatch.setattr(
        eg, "_priv", lambda argv, **k: subprocess.CompletedProcess(argv, 0, "", "")
    )
    eg.apply(
        "192.168.122.5",
        VmEgressPolicy(exit_driver="direct"),
        gateway="192.168.122.1",
        mac="52:54:00:aa:bb:cc",
    )
    dhcp = next(
        i
        for i, s in enumerate(issued)
        if "INPUT 1 -m mac --mac-source 52:54:00:aa:bb:cc -p udp --dport 67 -j ACCEPT"
        in s
    )
    anti = next(
        i
        for i, s in enumerate(issued)
        if "INPUT 1 -m mac --mac-source 52:54:00:aa:bb:cc ! -s 192.168.122.5 -j DROP"
        in s
    )
    assert (
        dhcp > anti
    )  # inserted LATER at pos 1 → ends up ABOVE the anti-spoof in the chain
...
def test_apply_installs_sibling_spoof_complement(monkeypatch):
    ...
    assert any(
        "FORWARD 1 -s 192.168.122.5 -m mac ! --mac-source 52:54:00:aa:bb:cc -j DROP"
        in s
        for s in issued
    )
    assert any(
        "INPUT 1 -s 192.168.122.5 -m mac ! --mac-source 52:54:00:aa:bb:cc -j DROP" in s
        for s in issued
    )
...
def test_v6_drop_raises_on_real_rejection(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_priv",
        lambda argv, **k: subprocess.CompletedProcess(
            argv, 1, "", "Bad rule (does a matching rule exist?)"
        ),
    )
    with pytest.raises(RuntimeError, match="v6 fail-closed"):
        eg._v6_drop("FORWARD", "52:54:00:aa:bb:cc")
...
def test_v6_drop_tolerates_absent_v6_stack(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda argv, **k: subprocess.CompletedProcess(
            ...
            3,
            "",
            "ip6tables v1.8: can't initialize ip6tables table `filter': Table does not exist",
        ),
    )
...
def test_forward_inetsim_sink_exempt_from_port_allowlist():
    ...
    flat = _flat(
        forward_chain_rules(
            "192.168.122.5", pol, gateway="192.168.122.1", sink_addr="172.28.100.1"
        )
    )
    ...
    assert sink_i < len(flat) - 1 and flat[-1].endswith(
        "-j DROP"
    )  # before the catch-all drop
...
def test_flush_conntrack_tolerates_missing_binary(monkeypatch):
    ...
    eg._flush_conntrack(
        "192.168.122.5"
    )  # conntrack(8) absent → best-effort, must not raise
...
def test_run_tolerates_missing_binary(monkeypatch):
    ...
    assert (
        cp.returncode == 127
    )  # nonzero → check=False deletes no-op, teardown continues
...
def test_remove_continues_to_ipv4_cleanup_when_ip6tables_absent(monkeypatch):
    ...
    assert any(
        a[0] == "ip6tables" for a in seen
    )  # v6 delete was attempted (and tolerated)
    assert any(
        a[0] == "iptables" and "-D" in a for a in seen
    )  # IPv4 cleanup STILL ran afterwards
...
def test_forward_inetsim_block_internal_exempts_sink():
    ...
    flat = _flat(
        forward_chain_rules(
            "192.168.122.9", pol, gateway="192.168.122.1", sink_addr="172.28.100.1"
        )
    )
    assert any("-d 172.28.100.1 -j ACCEPT" in f for f in flat)
    sink_i = next(i for i, f in enumerate(flat) if "-d 172.28.100.1 -j ACCEPT" in f)
...
def test_routing_tor_block_internal_returns_internal_before_redirect():
    ...
    flat = [
        " ".join(c)
        for c in routing_commands("192.168.122.5", "tor", rt, None, block_internal=True)
    ]
    ret = [i for i, f in enumerate(flat) if "-p tcp -j RETURN" in f]  # internal RETURNs
    red = [
        i
        for i, f in enumerate(flat)
        if "REDIRECT --to-ports" in f and "--dport 53" not in f
    ]
    assert (
        ret and red and max(ret) < min(red)
    )  # internal RETURNs precede the TransPort REDIRECT
...
def test_tor_teardown_inverts_block_internal_returns():
    td = [
        " ".join(c)
        for c in routing_teardown_commands(
            "192.168.122.5", "tor", ExitRouting(), None, block_internal=True
        )
    ]
...
def test_input_chain_direct_allows_host_resolver_dns_only():
    ...
    flat = _flat(
        input_chain_rules("192.168.122.9", pol, gateway="192.168.122.1", routing=None)
    )
    c = "BBVMIN_192_168_122_9"
    assert (
        flat[0] == f"-A {c} -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT"
    )  # agent return
    assert f"-A {c} -p udp --dport 67 -j ACCEPT" in flat  # DHCP renew
    assert (
        f"-A {c} -d 192.168.122.1 -p udp --dport 53 -j ACCEPT" in flat
    )  # host resolver
...
def test_input_chain_vpn_drops_host_resolver_dns():
    ...
    assert not any(
        "--dport 53" in f for f in flat
    )  # no host-resolver DNS exemption at all
...
def test_input_chain_none_reaches_no_host_service():
    flat = _flat(
        input_chain_rules(
            "10.0.0.5",
            VmEgressPolicy(exit_driver="none"),
            gateway="10.0.0.1",
            routing=None,
        )
    )
...
def test_input_chain_tor_accepts_redirect_targets():
    ...
    flat = _flat(
        input_chain_rules(
            "192.168.122.9",
            VmEgressPolicy(exit_driver="tor"),
            gateway="192.168.122.1",
            routing=rt,
        )
    )
...
def test_nat_chain_deletes_sweep_redirects_regardless_of_port():
    ...
    dels = _nat_chain_deletes(
        "192.168.122.50", "PREROUTING", ("REDIRECT", "DNAT"), dump
    )
    assert len(dels) == 3  # all .50 redirects regardless of --to-ports, not .5
    assert all("192.168.122.50/32" in " ".join(d) for d in dels)
    assert any(
        "9053" in " ".join(d) for d in dels
    )  # the DNS→DNSPort redirect is swept too
...
def test_direct_egress_is_not_tunnel_scoped():
    ...
    fwd = _flat(
        forward_chain_rules(
            "192.168.122.5", VmEgressPolicy(exit_driver="direct"), "192.168.122.1"
        )
    )
...
def test_routing_next_hop_shared_router_mode():
    ...
    assert any(
        "route replace default via 10.99.0.2 dev br-routers table 174260226" in c
        for c in cmds
    )
    ...
    assert any(
        "POSTROUTING -s 192.168.122.5 -o br-routers -j MASQUERADE" in c for c in cmds
    )
...
def test_routing_next_hop_table_id_avoids_collisions():
    ...
    def _table(gw):
        cmds = _flat(
            routing_commands(
                "192.168.122.5", "openvpn", ExitRouting(gateway=gw, leg="br-routers")
            )
        )
    ...
    assert len({_table(gw) for gw in gws}) == len(
        gws
    )  # all distinct, incl. the cross-/8 pair
...
def test_routing_next_hop_no_masquerade():
    ...
    assert any(
        "route replace default via 10.99.0.3 dev br-routers table 174260227" in c
        for c in cmds
    )
...
def test_routing_next_hop_teardown_keeps_shared_route():
    ...
    assert any(
        "POSTROUTING -s 192.168.122.5 -o br-routers -j MASQUERADE" in c and "-D" in c
        for c in rm
    )
