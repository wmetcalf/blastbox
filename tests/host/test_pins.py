...
def test_the_lock_header_states_the_extras_it_was_compiled_for(tmp_path):
    """uv writes a plain `blastbox==...` line even for `--extra host`.

    Inference cannot rescue an older lock that carries half a newly grown
    extra, but the header recorded the option outright.
    """
    from blastbox.host.pins import missing_from_locks
    ...
    # Only ONE of host's two dependencies -- below the inference threshold.
    _write(
        tmp_path,
        "req.lock",
        header
