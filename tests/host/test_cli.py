...
class TestDispatchArgparse:
    ...
    def test_dispatch_network_multi_engine_does_not_start_pool(self):
        ...
        with (
            ...
            patch(
                "blastbox.host.jobs.factory.build_job_store_from_env",
...
class TestPkiCli:
    def test_init_creates_ca_and_dispatcher_cert(self, tmp_path):
        ...
        assert (tmp_path / "dispatcher.crt").exists() and (
            tmp_path / "dispatcher.key"
        ).exists()

    def test_issue_server_is_ca_signed(self, tmp_path):
        main(["pki", "--dir", str(tmp_path), "init"])
        assert (
            main(["pki", "--dir", str(tmp_path), "issue-server", "--san", "10.0.0.5"])
            == 0
        )
    ...
    def test_sign_csr(self, tmp_path):
        ...
        csr = (
            ...
            .add_extension(
                x509.SubjectAlternativeName(
                    [x509.IPAddress(ipaddress.ip_address("10.0.0.9"))]
                ),
                critical=False,
            )
        ...
        )  # worker's key never left the box; dispatcher signed the CSR
    ...
    def test_import_ca_installs_pregenerated(self, tmp_path):
        ...
        assert (
            main(
                [
                    ...
                    "import-ca",
                    "--ca-cert",
                    str(tmp_path / "src.crt"),
                    "--ca-key",
                    str(tmp_path / "src.key"),
                ]
            )
            == 0
        )
        assert (
            pki / "ca.crt"
        ).read_bytes() == src.cert_pem  # the imported CA, not a fresh one
        assert oct((pki / "ca.key").stat().st_mode)[-3:] == "600"
        assert (
            main(["pki", "--dir", str(pki), "issue-client", "--cn", "d2"]) == 0
        )  # issues from it
        ca = x509.load_pem_x509_certificate((pki / "ca.crt").read_bytes())
        assert (
            x509.load_pem_x509_certificate((pki / "d2.crt").read_bytes()).issuer
            == ca.subject
        )
