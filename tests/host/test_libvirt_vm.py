...
def _pooled_rt(
    monkeypatch, pool: str = "192.168.122.200-192.168.122.201", **kw
) -> LibvirtVmRuntime:
    ...
    monkeypatch.setattr(
        mod, "_run", lambda *a, **k: type("C", (), {"returncode": 0, "stdout": ""})()
    )
...
def test_domain_xml_omits_emulator_by_default():
    ...
    assert "<emulator>/opt/q</emulator>" in _rt(emulator="/opt/q")._domain_xml(
        "bbvm-x", "/o.qcow2"
    )
...
def test_domain_xml_pins_mac_ip_via_nwfilter():
    ...
    assert "filter='custom-nf'" in _rt(nwfilter="custom-nf")._domain_xml(
        "bbvm-x", "/o.qcow2"
    )
    ...
    assert (
        "<filterref filter='clean-traffic'/>" in barex
        and "CTRL_IP_LEARNING" not in barex
    )
...
def test_vmslot_endpoint():
    ...
    assert (
        VmSlot(slot_id="b", domain="d", overlay="/o", agent_port=8765).endpoint is None
    )
...
def test_available_fail_closed_without_golden():
    ...
    assert (
        LibvirtVmRuntime(
            LibvirtVmConfig(golden_base="/no/such/golden.qcow2", sudo=False)
        ).available()
        is False
    )
...
def test_ip_pool_parse_and_mac_derivation():
    ...
    assert (
        _mac_for_ip("52:54:00:bb", "192.168.122.240") == "52:54:00:bb:7a:f0"
    )  # 122->7a, 240->f0
...
def test_dhcpserver_restricts_clean_traffic_dhcp_source(monkeypatch):
    ...
    xp = rt._domain_xml(
        "bbvm-x", "/o.qcow2", mac="52:54:00:bb:7a:c8", assigned_ip="192.168.122.200"
    )
    ...
    assert "DHCPSERVER" not in _rt(nwfilter="custom-nf")._domain_xml(
        "bbvm-x", "/o.qcow2"
    )
...
def test_domain_xml_assign_enforce_pins_explicit_mac_and_ip(monkeypatch):
    ...
    x = rt._domain_xml(
        "bbvm-x", "/o.qcow2", mac="52:54:00:bb:7a:f0", assigned_ip="192.168.122.240"
    )
...
def test_spawn_reserves_and_assigns_then_reap_unreserves(monkeypatch):
    ...
    assert any(
        a[:3] == ["net-update", "default", "add"] and slot.ip in " ".join(a)
        for a in calls
    )
    held = slot.ip
    calls.clear()
    rt.reap(slot)
    assert any(
        a[:3] == ["net-update", "default", "delete"] and held in " ".join(a)
        for a in calls
    )
...
def test_spawn_reserve_failure_does_not_delete_others_reservation(monkeypatch):
    ...
    def fake_virsh(*a, **k):
        ...
        if a[:3] == ("net-update", "default", "add"):
            return type(
                "C", (), {"returncode": 1, "stdout": "", "stderr": "already in use"}
            )()
        return _OK()
...
def test_reconcile_keeps_running_workers_ip_and_clears_stale(monkeypatch):
    ...
    def fake_run(args, **k):
        ...
        if "net-dumpxml" in s:
            return type(
                ...
                {
                    "returncode": 0,
                    "stdout": "<host mac='52:54:00:bb:7a:c8' ip='192.168.122.200'/>"  # running -> keep
                    "<host mac='52:54:00:bb:7a:c9' ip='192.168.122.201'/>",
                },  # stale   -> delete
            )()
    ...
    assert any("192.168.122.201" in d for d in deletes) and not any(
        "192.168.122.200" in d for d in deletes
    )
...
def test_dhcp_learning_mode_no_reservation(monkeypatch):
    ...
    slot = rt.spawn()
    assert (
        slot.ip is None and slot.mac == "52:54:00:aa:bb:cc"
    )  # learned later via DHCP/neigh
...
def test_available_fails_closed_when_helper_binary_missing(monkeypatch):
    ...
    assert (
        mod.LibvirtVmRuntime(
            mod.LibvirtVmConfig(golden_base="/g.qcow2", sudo=True)
        ).available()
        is False
    )
...
def test_alloc_overlay_name_uses_64_bits_and_unique_path(monkeypatch):
    ...
    rt = _rt()
    monkeypatch.setattr(
        rt, "_sh", lambda args, **k: type("C", (), {"returncode": 1})()
    )  # nothing exists
...
def test_alloc_overlay_name_rerolls_past_existing_overlay(monkeypatch):
    ...
    def fake_sh(args, **k):
        ...
        seen.append(overlay)
        return type(
            "C", (), {"returncode": 0 if len(seen) == 1 else 1}
        )()  # 1st exists, 2nd free

    monkeypatch.setattr(rt, "_sh", fake_sh)
    sid, name, overlay = rt._alloc_overlay_name()
    assert (
        len(seen) == 2 and overlay == seen[1] and overlay != seen[0]
    )  # re-rolled past the collision


def test_alloc_overlay_name_raises_when_no_free_name(monkeypatch):
    rt = _rt()
    monkeypatch.setattr(
        rt, "_sh", lambda args, **k: type("C", (), {"returncode": 0})()
    )  # all "exist"
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: type(
            ...
            (),
            {
                "returncode": 127,
                "stdout": "",
                "stderr": "sudo: virsh: command not found",
            },
        )(),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: type(
            ...
            (),
            {
                "returncode": 1,
                "stdout": "",
                "stderr": "error: failed to get domain 'bbvm-x'",
            },
        )(),
    )
...
def test_available_uses_privileged_existence_probe(monkeypatch):
    ...
    rt = (
        _rt()
    )  # golden_base=/dev/shm/golden.qcow2 — not present on the unprivileged fs here
    calls = []
    monkeypatch.setattr(
        rt, "_sh", lambda args, **k: calls.append(args) or _OK()
    )  # sudo test -e → rc0
    monkeypatch.setattr(mod, "_run", lambda *a, **k: _OK())  # virsh version → rc0
    assert rt.available() is True
...
def test_on_ready_fallback_fires_at_finalize_and_recycle_when_no_qemuga(monkeypatch):
    ...
    smoke_at: list[str] = []
    rt = _rt(
        on_ready=lambda slot: calls.append("ready"),
...
def test_recycle_raises_on_revert_failure(monkeypatch):
    ...
    monkeypatch.setattr(rt, "_port_up", lambda *a, **k: True)
    slot = VmSlot(
        slot_id="z", domain="bbvm-z", overlay="/o", agent_port=8765, ip="192.168.122.9"
    )
...
def test_trust_anchors_skipped_without_creds(monkeypatch):
    ...
    monkeypatch.setattr(
        mod.guest_ca, "install_trust_anchors", lambda *a, **k: called.append(1) or True
    )
    slot = VmSlot(
        slot_id="z", domain="bbvm-z", overlay="/o", agent_port=8765, ip="1.2.3.4"
    )
    rt.is_ready(slot)
    assert called == []
...
def _reap_rt(monkeypatch, *, destroyed: bool):
    ...
    order: list[str] = []
    monkeypatch.setattr(
        rt, "_destroy_domain", lambda name: order.append("destroy") or destroyed
    )
...
def test_reap_destroys_guest_before_removing_egress(monkeypatch):
    ...
    assert _RecEgress.events == [
        "egress-remove"
    ]  # policy unhooked after the guest is destroyed
...
def test_reap_keeps_egress_and_overlay_when_destroy_fails(monkeypatch):
    ...
    with pytest.raises(
        RuntimeError, match="quarantined"
    ):  # raise so the pool quarantines the slot
        rt.reap(slot)
    assert order == ["destroy"]  # bailed after the failed destroy
    assert (
        _RecEgress.events == []
    )  # egress LEFT in place — possibly-live guest stays contained
...
def test_f3_a_confirmed_missing_domain_is_dead_not_unknown(monkeypatch, stderr):
    ...
    slot = VmSlot(
        slot_id="w1",
        domain="bb-w1",
        overlay="/tmp/w1.qcow2",
        agent_port=8765,
        state=SlotState.IDLE,
    )
    assert rt.is_alive(slot) is False, (
        "a confirmed-absent domain must be reaped + replaced"
    )
...
def test_f3_an_unqueryable_control_plane_is_still_unknown_not_dead(monkeypatch, stderr):
    ...
    assert rt.is_alive(slot) is None, (
        "an unqueryable control plane is UNKNOWN, not a verdict"
    )
    assert rt.is_alive(slot) is not False, (
        "an unreachable libvirtd must NOT be read as a dead domain"
    )
    assert rt.is_alive_for_claim(slot) is None, (
        "unknown at hand-out means skip, not destroy"
    )
