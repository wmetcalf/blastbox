...
The route is mounted by ``build_app`` only when the store implements page-hash
search (the SQL store). These tests populate a sqlite store directly via
``upsert_page_hashes`` (the same rows the dispatcher's ``index_page_hashes``
writes on DONE), then drive the endpoint with a ``TestClient`` and assert match
semantics + validation + the memory-store no-route case.
"""
...
def _sql_store(tmp_path) -> SqlJobStore:
...
def test_route_present_for_sql_store(tmp_path):
    client = _client(_sql_store(tmp_path), tmp_path)
    # No data, but a well-formed query reaches the handler (empty result, not 404).
    resp = client.get("/v1/similar", params={"sha256": "a" * 64})
...
def test_phash_search_within_hamming(tmp_path):
    ...
    # phash is rendered back as the original 16-char hex
    assert {r["phash"] for r in results} == {base, near}
    assert all(r["filename"] == "doc.pdf" for r in results)


def test_colorhash_exact_match(tmp_path):
    store = _sql_store(tmp_path)
    _done_job_with_pages(
        store,
        filename="a.pdf",
...
def test_colorhash_fuzzy_total_cap(tmp_path):
    store = _sql_store(tmp_path)
    target = "0" * 14
...
def test_sha256_exact_match(tmp_path):
    store = _sql_store(tmp_path)
    _done_job_with_pages(
        store,
        filename="a.pdf",
...
def test_only_done_jobs_surface(tmp_path):
    store = _sql_store(tmp_path)
    _done_job_with_pages(
        store,
        filename="done.pdf",
...
def test_exactly_one_hash_required(tmp_path):
    client = _client(_sql_store(tmp_path), tmp_path)
    # zero provided
    assert client.get("/v1/similar").status_code == 400
    # two provided
    assert client.get(
...
def test_bad_phash_format_rejected(tmp_path):
    client = _client(_sql_store(tmp_path), tmp_path)
...
def test_out_of_range_params_rejected(tmp_path):
    client = _client(_sql_store(tmp_path), tmp_path)
    assert client.get(
