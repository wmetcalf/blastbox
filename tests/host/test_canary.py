...
class _WriteFails(LocalBlobStore):
    def put_output(self, job_id, out_dir):
        raise OSError(
            "Could not connect to the endpoint URL: http://172.18.101.15:9000"
        )
...
class _CorruptsBytes(LocalBlobStore):
    def open_output(self, job_id, name):
        ...
        return io.BytesIO(
            json.dumps({"status": "ok", "artifacts": [], "tampered": 1}).encode()
        )
...
def test_a_local_store_on_a_shared_queue_warns_by_default(tmp_path, caplog):
    ...
    assert any(
        "local_blob_store_with_shared_queue" in r.message for r in caplog.records
    )
...
def test_it_refuses_only_when_the_operator_declares_a_fleet(tmp_path):
    ...
    with pytest.raises(CanaryFailure) as ei:
        check_store_coherence(
            _Postgres(), _local(tmp_path), tmp_path / "jobs", require_shared=True
        )
    msg = str(ei.value)
...
def test_redis_counts_as_shared(tmp_path):
    ...
    with pytest.raises(CanaryFailure):
        check_store_coherence(
            _Redis(), _local(tmp_path), tmp_path / "jobs", require_shared=True
        )
...
def test_a_single_node_sqlite_deployment_never_even_warns(tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="blastbox.canary"):
        check_store_coherence(
            _Sqlite(), _local(tmp_path), tmp_path / "jobs", require_shared=True
        )
...
def test_an_unrecognised_job_store_never_triggers_the_failure(tmp_path):
    ...
    check_store_coherence(
        object(), _local(tmp_path), tmp_path / "jobs", require_shared=True
    )
...
def test_a_malformed_interval_never_silently_disables_the_periodic_pass(
...
def test_an_undeletable_store_leaves_ONE_object_not_one_per_probe(tmp_path):
    ...
    left = {
        p.relative_to(tmp_path / "blobs").parts[1]
...
def test_the_probe_uses_the_dispatchers_scratch_not_the_system_tmp(
...
@pytest.mark.parametrize(
    ...
    [
        ("-1", 900.0),
        ("-0.5", 900.0),  # finite, passes isfinite, then `> 0` silently disables
        ("0", 0.0),
        ("5", 5.0),
    ],
)
def test_a_negative_interval_does_not_silently_disable_the_periodic_pass(
    raw, expected, monkeypatch
):
    from blastbox.host.cli import _canary_settings
...
def test_the_shared_store_declaration_parses_the_documented_values(
    raw, expected, monkeypatch
):
    from blastbox.host.cli import _require_shared_blob_store
...
def test_the_serial_periodic_canary_is_not_blocked_by_a_long_job(tmp_path, monkeypatch):
    ...
    d._run_forever_serial = lambda *a, **k: released.wait(
        3.0
    )  # a job that blocks for "ages"
...
class _S3WithSecretEndpoint(_FakeS3):
    class _Client:
        class meta:  # noqa: N801
            endpoint_url = (
                "https://AKIAEXAMPLE:s3cr3t-p4ssw0rd@minio.internal:9000/?token=abc123"
            )
...
def test_endpoint_redaction_keeps_identity_and_drops_secrets(
    url, expect_in, expect_out
):
...
def test_the_network_canary_ticks_with_maintenance_disabled():
    ...
    st = _Stub()
    t = threading.Thread(
        target=VmJobDispatcher._maintenance_loop.__get__(st, _Stub), daemon=True
    )
    t.start()
    _time.sleep(0.5)
    st._stop.set()
...
def test_the_network_loop_sleeps_to_the_canary_deadline_not_the_floor():
    ...
    assert _CountingEvent.waits <= 3, (
        f"loop spun {_CountingEvent.waits} times instead of sleeping"
    )
...
def test_a_stores_exception_cannot_leak_the_endpoint_credentials(tmp_path):
    ...
    with pytest.raises(CanaryFailure) as ei:
        blob_roundtrip(
            _EndpointEchoingStore(tmp_path / "jobs", blob_root=tmp_path / "blobs")
        )
    msg = str(ei.value)
    ...
    assert ei.value.__cause__ is None, (
        "the raw exception must not be chained into tracebacks"
    )
...
def test_the_blob_target_is_logged_even_with_the_canary_disabled(tmp_path, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="blastbox.host.dispatch"):
        Dispatcher.run_forever(
            d, poll_interval_s=0.01, maintenance_interval_s=0, canary=False
        )
...
def test_a_slow_maintenance_sweep_cannot_delay_the_canary():
    ...
    st._stop.set()
    t.join(timeout=3)
    assert order and order[0] == "canary", (
        f"maintenance ran first and blocked the canary: {order}"
    )
...
def test_another_probes_bytes_on_a_shared_key_retries_rather_than_failing_a_boot(
    tmp_path,
):
...
def test_a_store_that_always_returns_wrong_bytes_still_fails(tmp_path):
    ...
    with pytest.raises(CanaryFailure) as ei:
        blob_roundtrip(
            _AlwaysForeignBytes(tmp_path / "jobs", blob_root=tmp_path / "blobs"),
            key_hint="fc",
        )
    assert "different bytes" in str(ei.value)
...
class _FakeVersionedS3:
    ...
    def put_output(self, job_id, out_dir) -> None:  # noqa: ANN001
        ...
        self.versions.append(
            (self._key("results", job_id) + "/metadata.json", f"v{self._n}")
        )
    ...
    def delete_job(self, job_id) -> None:  # noqa: ANN001
        ...
        self._n += 1
        self.versions.append(
            (self._key("results", job_id) + "/metadata.json", f"dm{self._n}")
        )
...
def test_required_shared_storage_is_enforced_even_with_the_canary_disabled(tmp_path):
    ...
    assert "SHARED queue" in str(ei.value)
    assert claimed == [], (
        "the dispatcher claimed work before the topology gate refused it"
    )
...
def test_the_coherent_case_still_starts_with_the_canary_disabled(tmp_path):
    ...
    d._require_shared_blob_store = True
    d.run_forever(
        poll_interval_s=0.001, maintenance_interval_s=0, stop=lambda: True, canary=False
    )  # must not raise
