...
def test_auto_created_share_dir_is_group_writable_not_world(tmp_path):
    ...
    assert mode & 0o070 == 0o070, (
        f"auto-created share dir not group-writable: {oct(mode)}"
    )
    assert not (mode & 0o007), (
        f"auto-created share dir must NOT be world-accessible: {oct(mode)}"
    )
...
def test_cold_only_gate_floored_on_fail_closed(tmp_path):
    ...
    share.publish(
        DemandSnapshot("clip", 5, 0, 4096, 1, 0, 8, 1.0, ts=1.0, node="", tier="cold")
    )
    ...
    cfg = NodeConfig(
        ...
        ram_headroom_frac=1.0,
        vcpu_oversubscription=999,
        stale_after_s=60,
    )
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=4096, max_ceiling=8),
        None,
        share,
        cfg,
        runtime="cold",
        ...
        node="n",
        instance="i",
        capacity_fn=_budget(8 * 4096, 999),
        clock=lambda: 1.0,
        concurrency_gate=gate,
        cold_slot_ram_mib=4096,
    )
...
def test_mixed_node_ids_fail_closed_to_floor(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=64, min_warm=8),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
        backlog_fn=lambda: 30,
        node="n",
        instance="c",
        capacity_fn=_budget(10 * 1024, 999),
        clock=lambda: 1.0,
    )
    mine = ds.tick()
    assert (
        mine.warm_size == 1 and mine.concurrent_ceiling == 1
    )  # ceiling 1, NOT min_warm=8
...
def test_budget_consensus_uses_min_across_view(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=64),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
        backlog_fn=lambda: 40,
        node="n",
        instance="c",
        capacity_fn=_budget(16 * 1024, 16),
        clock=lambda: 1.0,
    )
    mine = ds.tick()
...
def test_mixed_balancing_modes_on_a_node_do_not_oversubscribe(tmp_path):
    ...
    clip = DispatcherSizer(
        EngineNode(
            "clip", "-", slot_ram_mib=1024, max_ceiling=64, min_warm=0, weight=1.0
        ),
        clip_pool,
        share,
        NodeConfig(
    ...
    red = DispatcherSizer(
        EngineNode(
            "red", "-", slot_ram_mib=1024, max_ceiling=64, min_warm=0, weight=9.0
        ),
        red_pool,
        share,
        NodeConfig(
...
def test_static_mode_uses_weight_not_backlog(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=64, weight=4.0),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
...
def test_skips_non_node_runtime(tmp_path):
    ...
    ds = DispatcherSizer(
        ...
        share,
        cfg,
        runtime=RUNTIME_AWS_LAMBDA_MICROVM,
        backlog_fn=lambda: 50,
    )
...
def test_run_loop_ticks_and_stops(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
...
def test_same_engine_across_two_physical_nodes_sizes_independently(tmp_path):
    ...
    common = dict(
        ...
        backlog_fn=lambda: 5,
        instance="p1",
        capacity_fn=_budget(8 * 1024, 999),
        clock=lambda: 1.0,
    )
    ds2 = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=64),
    ...
    assert sorted(p.name for p in tmp_path.glob("*.json")) == [
        "clip@firecracker@toolz2@p1.json",
        "clip@firecracker@toolz3@p1.json",
    ]
...
def test_same_engine_two_tiers_on_one_node_are_distinct_pools(tmp_path):
    ...
    ds_gv = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=64),
        pool_gv,
        share,
        cfg,
        runtime=RUNTIME_GVISOR,
        **common,
    )
    ds_fc.tick()
    ds_gv.tick()
...
def test_sizer_drives_concurrency_gate_to_cold_headroom(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=6),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
    ...
    assert gate.limit == max(
        1, mine.concurrent_ceiling - mine.warm_size
    )  # gate == cold headroom
...
def test_cold_gate_reserves_resident_slots_not_just_target(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=8, min_warm=1),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
...
def test_cold_gate_priced_by_cold_worker_footprint(tmp_path):
    ...
    pool = _Pool()
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=2048, max_ceiling=8, min_warm=0),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
        ...
        node="n",
        instance="i",
        capacity_fn=_budget(8 * 2048, 999),
        clock=lambda: 1.0,
        concurrency_gate=gate,
        cold_slot_ram_mib=4096,
    )  # cold worker is 2x the warm slot
    mine = ds.tick()
...
def test_overlapping_replicas_split_budget_not_double(tmp_path):
    ...
    new.tick()
    assert sorted(p.name for p in tmp_path.glob("*.json")) == [
        "clip@firecracker@toolz2@new.json",
        "clip@firecracker@toolz2@old.json",
    ]
...
def test_default_instance_is_unique_per_process_not_pid(tmp_path):
    ...
    a = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024), _Pool(), share, cfg, **common
    )
    b = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024), _Pool(), share, cfg, **common
    )
...
def test_slow_publisher_refresh_hint_prevents_aging(tmp_path):
    ...
    assert [s.engine for s in share.read_all(max_age_s=20.0, now=30.0)] == [
        "red"
    ]  # kept (eff 70s)
...
def test_multi_engine_weight_clamped_to_valid_cap(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(
        "BLASTBOX_NODE_ENGINE_BB_WEIGHT", "600000"
    )  # sum 1.2M > _MAX_WEIGHT
    monkeypatch.setenv("BLASTBOX_NODE_SHARE_DIR", str(tmp_path))
...
def test_multi_engine_pool_ceiling_sums_caps_bounded_by_concurrency(
    ...
    monkeypatch.setenv("BLASTBOX_NODE_SHARE_DIR", str(tmp_path))
    res = _start_node_sizer(
        _Pool(), ["clip", "red"], InMemoryJobStore(), "firecracker", 16
    )
    assert res is not None
    stop, thread, sizer = res
    try:
...
def test_reservation_prices_cold_in_warm_slot_equivalents(tmp_path):
    ...
    pool = _Pool(
        assigned=0, slot_count=0
    )  # no warm residency — isolate the cold pricing
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=2048, max_ceiling=64),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
        ...
        node="n",
        instance="i",
        capacity_fn=_budget(64 * 2048, 999),
        clock=lambda: 1.0,
        concurrency_gate=gate,
        cold_slot_ram_mib=4096,
    )  # cold = 2x warm slot
...
def test_long_identity_hashed_into_filesystem_safe_key(tmp_path):
    ...
    round_tripped = share.read_all(
        max_age_s=60, now=1.0
    )  # anti-impersonation check still passes
    assert len(round_tripped) == 1 and round_tripped[0].node == long_node


def test_start_node_sizer_removes_snapshot_when_setup_aborts_after_publish(
    tmp_path, monkeypatch
):
...
def test_published_reservation_holds_resident_slots(tmp_path):
    ...
    pool = _Pool(
        assigned=0, slot_count=8
    )  # 8 VMs still resident, but no active work / backlog
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=64),
        pool,
        share,
        ...
        node="n",
        instance="i",
        capacity_fn=_budget(16 * 1024, 999),
        clock=lambda: 1.0,
    )
    ds.tick()
    mine = next(s for s in share.read_all(max_age_s=60, now=1.0) if s.engine == "clip")
...
def test_adaptive_only_direct_config_is_active(tmp_path):
    ...
    pool = _Pool()
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=8),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
...
def test_combined_ceiling_clamped_to_reader_bound(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BLASTBOX_NODE_SHARE_DIR", str(tmp_path))
    res = _start_node_sizer(
        _Pool(), ["clip", "red"], InMemoryJobStore(), "firecracker", 6000
    )
    assert res is not None
    stop, thread, sizer = res
    try:
        assert (
            sizer._engine.max_ceiling <= _MAX_CEILING_SANE
        )  # clamped (sum 6000 > 4096)
...
def test_node_manages_tier_gating(monkeypatch):
    ...
    assert _node_manages_tier(
        "cold"
    )  # cold is now managed pool-lessly (gate + reservation)
    assert not _node_manages_tier(
        "aws-ec2"
    )  # cloud tiers stay unmanaged (platform owns concurrency)
    ...
    class _Casc:
        ...
        def __init__(self, *names):
            self.tiers = [_T(n) for n in names]
    ...
    assert _node_manages_tier(
        "cascade", _Pool(_Casc("firecracker", "gvisor"))
    )  # all-local → managed
    assert not _node_manages_tier(
        "cascade", _Pool(_Casc("firecracker", "aws-ec2"))
    )  # off-node member → not
    monkeypatch.setenv("BLASTBOX_NODE_RESOURCE_MANAGEMENT", "0")
    monkeypatch.delenv("BLASTBOX_NODE_BALANCING", raising=False)
    assert not _node_manages_tier(
        "cascade", _Pool(_Casc("firecracker", "gvisor"))
    )  # RM off → unmanaged even all-local
...
def test_cold_only_dispatcher_gets_budgeted_gate_and_publishes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BLASTBOX_NODE_RESOURCE_MANAGEMENT", "1")
    monkeypatch.setenv(
        "BLASTBOX_NODE_ENGINE_CLIP_RAM_MIB", "4096"
    )  # cold worker footprint
    monkeypatch.setenv("BLASTBOX_NODE_ENGINE_CLIP_MAX_CEILING", "8")
    monkeypatch.setenv("BLASTBOX_NODE_SHARE_DIR", str(tmp_path))
    gate = DynamicConcurrencyGate(8)
    res = _start_node_sizer(
        None, ["clip"], InMemoryJobStore(), "cold", 8, gate, 4096
    )  # pool=None!
...
def test_all_local_cascade_is_sized_and_published(tmp_path, monkeypatch):
    ...
    class _CascRuntime:
        ...
        def __init__(self, *names):
            self.tiers = [_CascTier(n) for n in names]
    ...
    try:
        ...
        assert (
            mine[0].slot_ram_mib == 2048
        )  # sized by the engine's declared local footprint
        assert (
            pool.concurrent_ceiling >= 1
        )  # the warm pool was actually resized (managed)
    finally:
        stop.set()
        thread.join(2.0)
        sizer.remove_own_snapshot()
...
def test_cascade_warm_capped_at_surviving_capacity_frees_cold_headroom(
    tmp_path, monkeypatch
):
    ...
    class _CRuntime:  # gvisor overflow tier was unavailable at boot → only firecracker:4 survives
        ...
        def __init__(self):
            self.tiers = [_CTier("firecracker", 4)]

    monkeypatch.setenv("BLASTBOX_NODE_ENGINES", "clip")
    monkeypatch.setenv("BLASTBOX_NODE_RESOURCE_MANAGEMENT", "1")
    monkeypatch.setenv(
        "BLASTBOX_NODE_ENGINE_CLIP_RAM_MIB", "512"
    )  # small → budget easily fits 8
    monkeypatch.setenv(
        "BLASTBOX_NODE_ENGINE_CLIP_MAX_CEILING", "8"
    )  # configured for 8, runtime holds 4
    monkeypatch.setenv("BLASTBOX_NODE_SHARE_DIR", str(tmp_path))
    store = InMemoryJobStore()
    ...
    res = _start_node_sizer(
        pool, ["clip"], store, "cascade", 8, gate, 0.0
    )  # cold priced 1:1 (0.0)
    assert res is not None
    stop, thread, sizer = res
    try:
        ...
        assert gate.limit >= 2, (
            f"cold gate starved to {gate.limit} (should get the freed headroom)"
        )
    finally:
        stop.set()
        thread.join(2.0)
...
def test_start_node_sizer_caps_ceiling_at_concurrency(tmp_path, monkeypatch):
    ...
    for _ in range(50):
        store.create(
            Job.new(engine="clip", filename="x")
        )  # huge backlog → wants many slots
    pool = _Pool()
...
def test_start_node_sizer_returns_none_if_publish_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BLASTBOX_NODE_SHARE_DIR", str(tmp_path))
    assert (
        _start_node_sizer(_Pool(), ["clip"], InMemoryJobStore(), "firecracker", 4)
        is None
    )
...
def test_update_publish_is_fenced_when_stopped_mid_count(tmp_path):
    ...
    )  # heartbeat removed, update fenced → gone
...
def test_read_all_gcs_leaked_tmp_file(tmp_path):
    ...
    share = FileNodeShare(str(tmp_path))
    leaked = (
        tmp_path / ".engine@firecracker@n@x.json.abc123.tmp"
    )  # mkstemp-style dotfile
    leaked.write_text("{}")
...
def test_read_all_survives_huge_int_ts(tmp_path):
    ...
    (tmp_path / "big@firecracker@n@h.json").write_text(
        _json.dumps(
            {
                "engine": "big",
                "backlog": 1,
...
def test_node_isolation_ignores_foreign_host(tmp_path):
    ...
    share = FileNodeShare(str(tmp_path))
    share.publish(
        DemandSnapshot("red", 50, 0, 1024, 1, 0, 64, 1.0, ts=1.0, node="hostB")
    )  # other host
    pool = _Pool(assigned=0)
    cfg = NodeConfig(
        balancing=True,
    ...
    view = [
        s.engine
        for s in share.read_all(max_age_s=60, now=1.0)
        if s.node in ("", "hostA")
    ]
    assert view == ["clip"]
    _ = DS
...
def test_default_node_lets_containers_coordinate(tmp_path, monkeypatch):
    ...
    mine = ds.tick()
    assert (
        mine.concurrent_ceiling < 10
    )  # shares the node with red — NOT isolated to itself
...
def test_node_namespaced_files_dont_collide_across_hosts(tmp_path):
    ...
    share = FileNodeShare(str(tmp_path))
    share.publish(
        DemandSnapshot("clip", 3, 0, 1024, 1, 0, 64, 1.0, ts=1.0, node="hostA")
    )
    share.publish(
        DemandSnapshot("clip", 9, 0, 1024, 1, 0, 64, 1.0, ts=1.0, node="hostB")
    )
...
def test_split_cap_never_clips_incumbent_reservation(tmp_path):
    ...
    ds = DispatcherSizer(
        ...
        node="a",
        instance="a",
        capacity_fn=_budget(12 * 1024, 999),
        clock=lambda: 1.0,
    )
    mine = ds.tick()
    assert (
        mine.concurrent_ceiling >= 8
    )  # reservation preserved, NOT clipped to split-cap 4
...
def test_local_warm_floor_split_across_replicas(tmp_path):
    ...
    pool = _Pool()
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=8, min_warm=2),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
...
def test_per_engine_cap_and_floor_split_across_replicas(tmp_path):
    ...
    mine = ds.tick()
    assert (
        mine.concurrent_ceiling <= 4
    )  # split cap 8/2=4, NOT the full 8 despite the budget
...
def test_late_finishing_count_is_consumed_not_discarded(tmp_path):
    ...
    ds = DispatcherSizer(
        ...
        cfg,
        runtime=RUNTIME_FIRECRACKER,
        backlog_fn=lambda: block.wait(5.0) or 1,
        node="n",
        instance="i",
        capacity_fn=_budget(8 * 1024, 999),
    )
...
def test_backlog_remainder_distributed_not_floored_to_zero(tmp_path):
    ...
    pool = _Pool()
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=1024, max_ceiling=8, min_warm=0),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
    ...
    assert (
        mine.warm_size == 1
    )  # "a" (lowest-ranked) gets the +1, not floor(1/2)=0 for all
...
def test_untargeted_backlog_deduped_across_engine_tiers(tmp_path):
    ...
    def _ceiling(with_gvisor_peer):
        ...
        ds = DispatcherSizer(
            ...
            runtime="firecracker",
            backlog_fn=lambda: 10,
            untargeted_backlog_fn=lambda: 10,
            node="n",
            instance="f",
            capacity_fn=_budget(8 * 1024, 999),
            clock=lambda: 1.0,
    ...
    shared = _ceiling(
        with_gvisor_peer=True
    )  # fc + gvisor: untargeted demand split → 5 each
...
def test_shared_backlog_split_across_replicas(tmp_path):
    ...
    assert (
        pool.concurrent_ceiling <= 5
    )  # not the ~8 it'd take if the backlog weren't split
...
def test_publish_orphan_lease_prices_cold_workers(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode("clip", "-", slot_ram_mib=2048, max_ceiling=8),
        _Pool(),
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
    ...
    mine = next(
        s for s in share.read_all(max_age_s=20.0, now=1100.0) if s.engine == "clip"
    )
...
def test_publish_orphan_lease_extends_reservation_lifetime(tmp_path):
    ...
    live = [
        s
        for s in share.read_all(max_age_s=20.0, now=1000.0 + 200)
        if s.engine == "clip"
    ]
    ...
    assert [
        s
        for s in share.read_all(max_age_s=20.0, now=1000.0 + 400)
        if s.engine == "clip"
    ] == []
...
def test_staleness_uses_publisher_declared_window_not_reader(tmp_path):
    ...
    share.publish(
        DemandSnapshot(
            "red", 5, 0, 1024, 1, 0, 64, 1.0, ts=100.0, node="n", stale_after_s=50.0
        )
    )
...
def test_node_filename_mismatch_is_rejected(tmp_path):
    ...
    share = FileNodeShare(str(tmp_path))
    (tmp_path / "clip@hostA.json").write_text(
        _json.dumps(
            {
                "engine": "clip",
                "backlog": 1,
    ...
    )  # filename says hostA, payload says hostB
...
def test_valid_bounds_reject_overflow_and_infinite_ts(tmp_path):
    ...
    (tmp_path / "big.json").write_text(
        _json.dumps(
            {  # 400-digit backlog → float() overflow
                "engine": "big",
                "backlog": int("9" * 400),
                "assigned": 0,
                "slot_ram_mib": 1024,
    ...
    assert {s.engine for s in share.read_all(max_age_s=60, now=1.0)} == {
        "good"
    }  # no crash
...
def test_partial_node_config_still_coordinates(tmp_path):
    ...
    share = FileNodeShare(str(tmp_path))
    share.publish(
        DemandSnapshot("red", 40, 0, 1024, 1, 0, 64, 1.0, ts=1.0, node="hostA")
    )
    pool = _Pool(assigned=0)
    cfg = NodeConfig(
        balancing=True,
    ...
    assert (
        mine.concurrent_ceiling < 10
    )  # sees red → shares, not isolated to the whole node
...
def test_static_mode_warm_tracks_real_demand_not_weight(tmp_path):
    ...
    ds = DispatcherSizer(
        EngineNode(
            "clip", "-", slot_ram_mib=1024, max_ceiling=64, weight=8.0, min_warm=0
        ),
        pool,
        share,
        cfg,
        runtime=RUNTIME_FIRECRACKER,
        backlog_fn=lambda: 0,
        capacity_fn=_budget(32 * 1024, 999),
        clock=lambda: 1.0,
    )
    mine = ds.tick()
...
def test_adaptive_sheds_below_half_under_sustained_pressure(tmp_path):
    ...
    ds = DispatcherSizer(
        ...
        runtime=RUNTIME_FIRECRACKER,
        backlog_fn=lambda: 1,
        avail_fn=lambda: 100.0,
    )  # far below min_free → real pressure
...
def test_adaptive_never_exceeds_physical_ram(tmp_path):
    ...
    )  # tons of free RAM → wants to ramp up
    base = NodeBudget(ram_mib=1000.0, vcpus=10.0)
    for _ in range(50):
        out = ds._adapt(base)
    ...
    assert (
        1.24 <= out2.ram_mib / base.ram_mib <= 1.25
    )  # allowed up to 1.25×, no further
...
def test_demand_snapshot_field_order_is_append_only():
    ...
    snap = DemandSnapshot(
        ...
        64,
        1.0,
        100.0,  # engine..ts (9 positional)
        "node-x",
        "firecracker",
        5.0,
        "inst-1",
        True,  # node, tier, refresh_s, instance, balancing
    )
    assert snap.engine == "eng" and snap.backlog == 10 and snap.ts == 100.0
    assert (
        snap.node == "node-x"
        and snap.tier == "firecracker"
        and snap.refresh_s == 5.0
        and snap.instance == "inst-1"
        and snap.balancing is True
    )
    ...
    assert (
        snap.budget_ram_mib == 0.0
        and snap.budget_vcpus == 0.0
        and snap.stale_after_s == 0.0
    )
...
def test_sizer_warns_when_min_warm_floor_starved(tmp_path, caplog):
    ...
    starved_warns = [
        r.getMessage() for r in caplog.records if "blastbox#68" in r.getMessage()
    ]
    ...
    assert any(
        "engine=red" in m and "configured min_warm=8" in m for m in starved_warns
    ), starved_warns
...
def test_no_floor_warning_for_cold_only_dispatcher(tmp_path, caplog):
    ...
    cold = DispatcherSizer(
        ...
        None,
        share,
        NodeConfig(**common),  # pool=None → cold-only
        runtime="cold",
        backlog_fn=lambda: 0,
        node="n",
        instance="c",
