...
def test_worker_caps_set_but_empty_env_use_defaults(tmp_path, monkeypatch):
    ...
    fail at launch with ``invalid argument "" for "--memory"`` (RC 125). Set-but-empty MUST fall
    back to the default exactly like unset — and no empty token may ever land in a value position."""
    for var in (
