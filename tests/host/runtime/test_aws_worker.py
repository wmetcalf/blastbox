...
def _cp(rc: int = 0, stdout: str = "", stderr: str = "") -> subprocess.CompletedProcess:
    return subprocess.CompletedProcess(
        args=["aws"], returncode=rc, stdout=stdout, stderr=stderr
    )
...
_IDENT = {
    "sts get-caller-identity": {
        "Account": "380038281108",
        "Arn": "arn:aws:iam::x:user/y",
    }
}


def _lambda_rt(responses, probe=lambda url, hdrs, to: True, **kw):  # noqa: ANN001
    cfg = LambdaMicroVmConfig(
        region="us-east-1",
        image_identifier="arn:aws:lambda:us-east-1:aws:microvm-image:al2023-1",
        allow_default_egress=True,
    )
    fake = FakeAws({**_IDENT, **responses})
    return LambdaMicroVmRuntime(
        cfg, aws_runner=fake, http_probe=probe, clock=lambda: 100.0, **kw
    ), fake
...
def _ec2_rt(responses, probe=lambda url, hdrs, to: True, **kw):  # noqa: ANN001
    ...
    fake = FakeAws({**_IDENT, **responses})
    return DisposableEc2Runtime(
        cfg, aws_runner=fake, http_probe=probe, clock=lambda: 100.0, **kw
    ), fake


def _snapstart_rt(
    responses, probe=lambda url, hdrs, to: True, clock=lambda: 100.0, **cfgkw
):  # noqa: ANN001
    cfg = LambdaSnapStartConfig(
        region="us-east-1",
        image_identifier="arn:img",
        allow_default_egress=True,
        resume_poll_s=0.0,
        **cfgkw,
    )
    ...
    return LambdaSnapStartRuntime(
        cfg, aws_runner=fake, http_probe=probe, clock=clock
    ), fake
...
def test_snapstart_is_alive_true_for_suspended_slot():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": {"state": "suspended"},
...
def test_snapstart_is_alive_does_not_mint_token_on_parked_slot():
    ...
    )  # far past any token half-TTL
    slot = AwsWorkerSlot(slot_id="s", resource_id="mv-1")
    slot.auth_token, slot.token_minted_at = (
        "aged",
        0.0,
    )  # a stale token that base is_alive would try to refresh
    for _ in range(5):
        rt.is_alive(slot)
    assert "lambda-microvms create-microvm-auth-token" not in [
        k for k, _ in fake.calls
    ]  # no mint storm
...
def test_snapstart_readiness_describe_is_cached_during_warming():
    ...
    rt, fake = _snapstart_rt(
        {"lambda-microvms get-microvm": {"state": "pending"}},  # no endpoint yet
        probe=lambda u, h, t: False,
        clock=lambda: 500.0,
    )  # constant clock -> within TTL
    slot = AwsWorkerSlot(slot_id="s", resource_id="mv-1")
    for _ in range(10):
        rt.is_ready(slot)
    ...
    assert (
        n_desc == 1
    )  # 10 readiness ticks within the cache window -> a single control-plane describe
...
def test_snapstart_resume_wakes_and_refreshes():
    ...
    assert (
        "lambda-microvms resume-microvm" in ops
    )  # nudged the parked slot awake after the probe failed
...
def test_snapstart_warmup_throttles_failing_token_mint():
    ...
    rt, fake = _snapstart_rt(
        {
            "lambda-microvms get-microvm": {
                "state": "pending",
                "endpoint": "vm.example",
            },  # endpoint pre-running
            "lambda-microvms create-microvm-auth-token": _cp(
                rc=254, stderr="microvm not running"
            ),
        },
    ...
    for _ in range(
        10
    ):  # 10 ticks 0.1s apart (~1s); throttle window = max(poll,1.0)=1.0
        assert rt.is_ready(slot) is False  # mint fails (pending) -> not ready
        clk["t"] += 0.1
    mints = sum(
        1 for k, _ in fake.calls if k == "lambda-microvms create-microvm-auth-token"
    )
...
def test_snapstart_throttles_failing_refresh_with_cached_token():
    ...
    rt, fake = _snapstart_rt(
        ...
        probe=lambda u, h, t: True,
        clock=lambda: clk["t"],
        auth_token_ttl_min=10,
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="mv-1")
    slot.auth_token = "AGED"  # a cached token...
    slot.token_minted_at = (
        -1e9
    )  # ...far past half-TTL, so _ensure_token always tries to re-mint
    for _ in range(10):
    ...
    assert (
        mints <= 2
    )  # throttled DESPITE the cached token (was 10 -- one per tick -- before the fix)
...
def test_snapstart_resume_remints_token_after_wake():
    ...
    def mint(argv):
        ...
        return _cp(
            stdout=json.dumps(
                {"authToken": "jwe-parked" if minted["n"] == 1 else "jwe-awake"}
            )
        )

    def probe(u, h, t):
        return (
            h.get("X-aws-proxy-auth") == "jwe-awake"
        )  # only an awake-minted token is accepted
    ...
    assert (
        slot.auth_token == "jwe-awake"
    )  # the awake token survived, not the parked one
    assert (
        minted["n"] >= 2
    )  # re-minted after the wake instead of reusing the parked JWE
...
def test_snapstart_resume_skips_resume_when_already_reachable():
    ...
    slot = AwsWorkerSlot(slot_id="s", resource_id="mv-1")
    rt.resume(slot)
    assert "lambda-microvms resume-microvm" not in [
        k for k, _ in fake.calls
    ]  # probe-first, already reachable
...
def test_snapstart_resume_wakes_stale_running_state_when_auto_resume_off():
    ...
    slot = AwsWorkerSlot(slot_id="s", resource_id="mv-1")
    rt.resume(slot)
    assert "lambda-microvms resume-microvm" in [
        k for k, _ in fake.calls
    ]  # woke despite 'running' state
...
def test_snapstart_config_from_env():
    ...
    assert (
        cfg.image_identifier == "arn:img" and cfg.allow_default_egress is True
    )  # inherited fields
...
def test_snapstart_config_clamps_to_aws_bounds():
    ...
    cfg = LambdaSnapStartConfig(
        ...
        allow_default_egress=True,
        max_idle_duration_s=30,
        suspended_duration_s=-5,
...
def test_snapstart_from_env_override_applies_without_collision():
    ...
    cfg = LambdaSnapStartConfig.from_env(
        {
            "BLASTBOX_LAMBDA_IMAGE": "img",
            "BLASTBOX_LAMBDA_ALLOW_DEFAULT_EGRESS": "1",
        }.get,
        resume_timeout_s=99.0,
        region="us-west-2",
    )
    assert (
        cfg.resume_timeout_s == 99.0 and cfg.region == "us-west-2"
    )  # base + new fields overridable


def test_snapstart_inherits_egress_failclosed():
    with pytest.raises(AwsUnavailable, match="egress"):
        LambdaSnapStartRuntime(
            LambdaSnapStartConfig(region="us-east-1", image_identifier="img")
        )
    ...
    rt, fake = _lambda_rt(
        {
            ...
            "lambda-microvms get-microvm": {
                "state": "running",
                "url": "https://mv-123.lambda-url.aws/",
            },
...
def test_lambda_requires_image_identifier():
    with pytest.raises(AwsUnavailable):
        LambdaMicroVmRuntime(
            LambdaMicroVmConfig(region="us-east-1", image_identifier="")
        )
    ...
    rt, _ = _ec2_rt(
        {
            ...
            "ec2 describe-instances": {
                "Reservations": [
                    {"Instances": [{"InstanceId": "i-9", "State": {"Name": "pending"}}]}
                ]
            },
        }
    )
...
def test_available_false_when_no_account():
    cfg = LambdaMicroVmConfig(
        region="us-east-1", image_identifier="img", allow_default_egress=True
    )
...
def test_lambda_refuses_default_egress_without_optin():
    ...
    with pytest.raises(AwsUnavailable, match="egress"):
        LambdaMicroVmRuntime(
            LambdaMicroVmConfig(region="us-east-1", image_identifier="img")
        )
...
def test_lambda_mint_token_accepts_string_and_map():
    ...
    rt_s, _ = _lambda_rt(
        {"lambda-microvms create-microvm-auth-token": {"authToken": "jwe-string"}}
    )
...
def test_lambda_is_alive_for_claim_refreshes_token():
    ...
    rt = LambdaMicroVmRuntime(
        cfg, aws_runner=fake, http_probe=lambda u, h, t: True, clock=lambda: clock["t"]
    )
    slot = rt._launch()
...
def test_lambda_claim_fails_when_token_unrefreshable():
    ...
    slot.token_minted_at = (
        -1e9
    )  # far past half-TTL -> _ensure_token re-mints (and fails)
    assert rt.is_alive_for_claim(slot) is not True, (
        "a slot with an unmintable token must not be handed out"
    )
    assert rt.is_alive_for_claim(slot) is None, (
        "an UNRECOGNISED mint failure is unknown, not death"
    )

    rt2, _ = _lambda_rt(
        {
            "lambda-microvms run-microvm": {"microvmId": "mv-2"},
            "lambda-microvms get-microvm": {
                "state": "running",
                "endpoint": "vm.example",
            },
            "lambda-microvms create-microvm-auth-token": _cp(
                rc=254, stderr="ResourceNotFoundException: microvm not found"
            ),
        }
    )
    slot2 = rt2._launch()
    ...
    assert rt2.is_alive_for_claim(slot2) is False, (
        "a CONFIRMED dead resource must still drop the slot"
    )
...
def test_snapstart_claim_does_not_fail_on_parked_token():
    ...
    rt, fake = _snapstart_rt(
        {
            "lambda-microvms get-microvm": {
                "state": "suspended"
            },  # parked -> alive via the base whitelist
            "lambda-microvms create-microvm-auth-token": _cp(
                rc=254, stderr="not running"
            ),
        },
        probe=lambda u, h, t: True,
    )
    ...
    assert (
        rt.is_alive_for_claim(slot) is True
    )  # base check (suspended alive), no token touch
    assert (
        sum(
            1 for k, _ in fake.calls if k == "lambda-microvms create-microvm-auth-token"
        )
...
def test_default_aws_runner_disables_pager(monkeypatch):
    ...
    assert (
        captured["env"]["AWS_ACCESS_KEY_ID"] == "sentinel"
    )  # creds preserved (os.environ spread)
...
def test_is_alive_for_claim_bypasses_liveness_cache():
    ...
    rt, _ = _lambda_rt(
        {
            "lambda-microvms run-microvm": {"microvmId": "mv-1"},
            "lambda-microvms get-microvm": lambda argv: _cp(
                stdout=json.dumps({"state": state["v"]})
            ),
        }
    )
    slot = rt._launch()
    ...
    state["v"] = "terminated"
    assert (
        rt.is_alive(slot) is True
    )  # STILL cached True (constant clock, within the 5s TTL)
...
def test_lambda_health_describe_is_cached_across_warming_ticks():
    ...
    rt, fake = _lambda_rt(
        {
            "lambda-microvms run-microvm": {"microvmId": "mv-1"},
            "lambda-microvms get-microvm": {"state": "pending"},
        },  # never resolves -> url stays None
    )
    slot = rt._launch()
    for _ in range(10):
    ...
    fake = FakeAws(
        {**_IDENT, "ec2 describe-instances": _cp(rc=254, stderr="AccessDenied")}
    )
...
def test_ec2_self_terminate_bool_spellings():
    ...
    for val in ("False", "FALSE", "NO", "No", "off", "Off", " false ", "0", "no"):
        cfg = Ec2Config.from_env(
            {"BLASTBOX_EC2_SELF_TERMINATE": val, "BLASTBOX_EC2_AMI": "ami-x"}.get
        )
    ...
    for val in (
        ...
        "on",
        "",
    ):  # "" -> _env default "1"; unset also defaults ON
        cfg = Ec2Config.from_env(
            {"BLASTBOX_EC2_SELF_TERMINATE": val, "BLASTBOX_EC2_AMI": "ami-x"}.get
        )
        assert cfg.self_terminate is True, val
    assert (
        Ec2Config.from_env({"BLASTBOX_EC2_AMI": "ami-x"}.get).self_terminate is True
    )  # unset -> ON
...
def test_snapstart_auto_resume_off_disables():
    ...
    for val in ("off", " Off ", "OFF"):
        cfg = LambdaSnapStartConfig.from_env(
            {
                "BLASTBOX_LAMBDA_SNAPSTART_AUTO_RESUME": val,
                "BLASTBOX_LAMBDA_IMAGE": "arn:img",
            }.get
        )
        assert cfg.auto_resume is False, val
    assert (
        LambdaSnapStartConfig.from_env(
...
def test_ec2_public_ip_requests_associate_flag():
    ...
    fake = FakeAws(
        {**_IDENT, "ec2 run-instances": {"Instances": [{"InstanceId": "i-1"}]}}
    )
    rt = DisposableEc2Runtime(
        ...
        aws_runner=fake,
        http_probe=lambda u, h, t: True,
        clock=lambda: 1.0,
    )
    rt.spawn()
    assert "--associate-public-ip-address" in next(
        a for k, a in fake.calls if k == "ec2 run-instances"
    )
    fake2 = FakeAws(
        {**_IDENT, "ec2 run-instances": {"Instances": [{"InstanceId": "i-2"}]}}
    )
    rt2 = DisposableEc2Runtime(
        Ec2Config(region="us-east-1", image_id="ami-x"),  # private-IP default
        aws_runner=fake2,
        http_probe=lambda u, h, t: True,
        clock=lambda: 1.0,
    )
    rt2.spawn()
    assert "--associate-public-ip-address" not in next(
        a for k, a in fake2.calls if k == "ec2 run-instances"
    )
...
def test_ec2_public_ip_without_tls_fails_closed():
    ...
    with pytest.raises(AwsUnavailable, match="cleartext"):
        DisposableEc2Runtime(
            Ec2Config(region="us-east-1", image_id="ami-x", use_public_ip=True),
            aws_runner=fake,
        )
    with pytest.raises(
        AwsUnavailable, match="cleartext"
    ):  # hibernate tier too (shared __init__)
        Ec2HibernateRuntime(
            Ec2HibernateConfig(
                region="us-east-1", image_id="ami-x", use_public_ip=True
            ),
            aws_runner=fake,
        )
    ...
    DisposableEc2Runtime(
        Ec2Config(region="us-east-1", image_id="ami-x"), aws_runner=fake
    )
...
def test_inject_tls_installs_mtls_probe_for_explicit_context():
    ...
    assert kw["ssl_context"] is ctx and callable(
        kw.get("http_probe")
    )  # TLS-aware probe installed
    ...
    assert "http_probe" not in _inject_tls(
        {}.get, {"ssl_context": None}
    )  # None stays http (no probe)
...
def test_ec2_public_ip_bool_spellings():
    ...
    for val in ("true", "YES", "On", " 1 "):
        assert Ec2Config.from_env(
            {"BLASTBOX_EC2_PUBLIC_IP": val, "BLASTBOX_EC2_AMI": "ami-x"}.get
        ).use_public_ip
    for val in ("0", "false", "off", ""):
        assert not Ec2Config.from_env(
            {"BLASTBOX_EC2_PUBLIC_IP": val, "BLASTBOX_EC2_AMI": "ami-x"}.get
        ).use_public_ip
...
def test_lambda_config_clamps_to_aws_bounds():
    ...
    hi = LambdaMicroVmConfig(
        region="us-east-1",
        image_identifier="arn:img",
        max_duration_s=999999,
        auth_token_ttl_min=120,
    )
    assert hi.max_duration_s == 28800 and hi.auth_token_ttl_min == 60
    lo = LambdaMicroVmConfig(
        region="us-east-1",
        image_identifier="arn:img",
        max_duration_s=0,
        auth_token_ttl_min=0,
    )
    assert lo.max_duration_s == 1 and lo.auth_token_ttl_min == 1
    # SnapStart chains the same clamp via super().__post_init__()
    snap = LambdaSnapStartConfig(
        region="us-east-1", image_identifier="arn:img", max_duration_s=999999
    )
...
def test_snapstart_runtime_satisfies_slotruntime_protocol():
    ...
    assert callable(
        getattr(rt, "resume", None)
    )  # the warm-claim seam the dispatcher detects
...
def test_pool_config_registers_snapstart(monkeypatch):
    ...
    ss, _ = _snapstart_rt({})
    monkeypatch.setattr(
        "blastbox.host.runtime.aws_worker.select_lambda_snapstart_runtime",
        lambda **kw: ss,
    )
    got = pool_config.select_runtime_by_name(
        "aws-lambda-snapstart", require_available=False
    )
...
def test_lambda_cli_arg_shapes_match_live_api():
    ...
    cfg = LambdaMicroVmConfig(
        region="us-east-1",
        image_identifier="arn:img",
        egress_connector_ids=("e-1", "e-2"),
        ingress_connector_ids=("i-1",),
        auth_token_ttl_min=25,
        agent_port=8765,
    )
    fake = FakeAws(
        {
    ...
    rt = LambdaMicroVmRuntime(
        cfg, aws_runner=fake, http_probe=lambda u, h, t: True, clock=lambda: 1.0
    )
    slot = rt.spawn()
    ...
    rt._mint_token(slot)
    tok = next(
        a for k, a in fake.calls if k == "lambda-microvms create-microvm-auth-token"
    )
    assert "--microvm-identifier" in tok
    assert tok[tok.index("--expiration-in-minutes") + 1] == "25"
    assert (
        tok[tok.index("--allowed-ports") + 1] == "port=8765"
    )  # tagged-union list shorthand
...
def test_ec2_forwards_agent_token():
    ...
    rt = DisposableEc2Runtime(
        cfg,
        aws_runner=fake,
        http_probe=lambda u, h, t: seen.append(h) or True,
        clock=lambda: 1.0,
    )
    slot = rt.spawn()
    ...
    assert rt.is_ready(slot) is True
    assert (
        seen[-1].get("X-aws-proxy-auth") == "tok123"
    )  # and sent in the readiness probe
...
def _hibernate_rt(*, state, healthy, clock=lambda: 100.0, **cfgkw):  # noqa: ANN001
    ...
    rt = Ec2HibernateRuntime(
        cfg, aws_runner=fake, http_probe=lambda u, h, t: healthy[0], clock=clock
    )
    return rt, fake
...
def test_ec2_hibernate_launch_adds_hibernation_and_encrypted_ebs():
    ...
    joined = " ".join(argv)
    assert (
        '"Encrypted": true' in joined or '"Encrypted":true' in joined
...
def test_ec2_hibernate_state_machine_parks_a_warmed_slot():
    ...
    assert (
        sum(1 for k, a in fake.calls if k == "ec2 stop-instances") == n_hib
    )  # issued exactly once
...
def test_ec2_hibernate_tolerates_not_ready_and_throttles():
    ...
    assert (
        sum(1 for k, a in fake.calls if k == "ec2 stop-instances") == 1
    )  # no new attempt
    ticks[0] += 10.0
    hib_ok[0] = True
...
def test_ec2_hibernate_resume_starts_and_health_gates():
    ...
    rt.resume(slot)
    assert any(
        k == "ec2 start-instances" for k, a in fake.calls
    )  # woke the hibernated instance
...
def test_ec2_hibernate_resume_times_out():
    ...
    rt, _ = _hibernate_rt(
        state=["stopped"], healthy=[False], clock=clock, resume_timeout_s=5.0
    )
    with pytest.raises(AwsWorkerError, match="not ready"):
        rt.resume(AwsWorkerSlot(slot_id="s", resource_id="i-1"))
...
def test_ec2_hibernate_config_defaults_self_terminate_on():
    ...
    off = Ec2HibernateConfig.from_env(
        {"BLASTBOX_EC2_AMI": "ami-x", "BLASTBOX_EC2_SELF_TERMINATE": "0"}.get
    )
    assert off.self_terminate is False
    assert cfg.root_volume_gb == 30 and cfg.root_device_name == "/dev/xvda"
    assert (
        cfg.ready_timeout_s == 600.0 and cfg.resume_timeout_s == 180.0
    )  # covers boot+warm+hibernate; < worker budget
...
def test_ec2_hibernate_backstop_is_uptime_based():
    ...
    rt = Ec2HibernateRuntime(
        Ec2HibernateConfig(region="us-east-1", image_id="ami-x", max_duration_s=3600),
        aws_runner=fake,
        http_probe=lambda u, h, t: True,
        clock=lambda: 1.0,
    )
    rt._launch()
    assert (
        "systemd-run --on-active=3600s /sbin/shutdown -h now" in captured["ud"]
    )  # uptime timer
...
def test_ec2_hibernate_preflight_rejects_incapable_type():
    ...
    fake.responses["ec2 describe-instance-types"] = {
        "InstanceTypes": [
            {"HibernationSupported": False, "MemoryInfo": {"SizeInMiB": 2048}}
        ]
    }
    fake.responses["ec2 describe-instances"] = {
        "Reservations": []
    }  # available()'s probe
    assert (
        rt.available() is False
    )  # _service_available raises -> available() fail-closed


def test_ec2_hibernate_preflight_rejects_undersized_root_volume():
    rt, fake = _hibernate_rt(
        state=["running"], healthy=[False], root_volume_gb=1
    )  # 1GB < 2GB RAM
    fake.responses["ec2 describe-instance-types"] = {
        "InstanceTypes": [
            {"HibernationSupported": True, "MemoryInfo": {"SizeInMiB": 2048}}
        ]
    }
...
def test_ec2_hibernate_resume_refreshes_public_ip_uncached():
    ...
    cfg = Ec2HibernateConfig(
        ...
        resume_poll_s=0.0,
        allow_plaintext_public=True,
    )  # this test exercises IP refresh, not the TLS guard
    fake = FakeAws(
        {
            **_IDENT,
            "ec2 describe-instances": describe,
    ...
    rt = Ec2HibernateRuntime(
        cfg,
        aws_runner=fake,
        http_probe=lambda u, h, t: "2.2.2.2" in u,
        clock=lambda: 100.0,
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="i-1")
...
def test_ec2_hibernate_select_and_registration():
    ...
    with m.patch(
        "blastbox.host.runtime.aws_worker.select_ec2_hibernate_runtime",
        lambda **kw: hib,
    ):
        got = pool_config.select_runtime_by_name(
            "aws-ec2-hibernate", require_available=False
        )
    assert got is hib
...
def test_ec2_self_terminate_ttl_injected():
    ...
    assert (
        "shutdown -h +10" in wrapped and "echo hi" in wrapped
    )  # 600s -> 10min; operator part kept
    # CEILING, not floor: a non-minute budget must never schedule the shutdown BEFORE max_duration_s.
    assert "shutdown -h +2" in _userdata_with_self_terminate(
        None, 119
    )  # 119s -> 2min (not 1)
    assert "shutdown -h +1" in _userdata_with_self_terminate(
        None, 60
    )  # exact minute stays 1
    ...
    assert "systemd-run --on-active=1s" in _userdata_with_self_terminate(
        None, -5, uptime=True
    )
    assert "systemd-run --on-active=1s" in _userdata_with_self_terminate(
        None, 0, uptime=True
    ...
    cfg = Ec2Config(
        region="us-east-1", image_id="ami-x", user_data_b64=ud, max_duration_s=1800
    )
    captured = {}

    def _run(
        argv,
    ):  # user-data is passed as a 0600 file:// (out of argv) -> read it DURING the call
        val = argv[argv.index("--user-data") + 1]
    ...
    argv = next(a for k, a in fake.calls if k == "ec2 run-instances")
    assert not any(
        "start-agent" in str(a) for a in argv
    )  # the bootstrap secret never hits argv (/proc)
    assert (
        "shutdown -h +30" in captured["ud"] and "start-agent" in captured["ud"]
    )  # ...but is in the file
...
def test_userdata_preserves_cloudinit_part_types():
    ...
    cases = [
        ...
        (
            "#cloud-config\nruncmd:\n  - x\n",
            "text/cloud-config",
        ),  # cloud-config unchanged
    ]
    for raw, expected in cases:
        parts = message_from_string(
            _userdata_with_self_terminate(raw, 600)
        ).get_payload()
        assert parts[0].get_content_type() == expected, (
            raw
        )  # operator part keeps its type
        assert (
            parts[1].get_content_type() == "text/x-shellscript"
        )  # TTL part stays a shell script
...
def test_ec2_self_terminate_preserves_multipart_userdata():
    ...
    payloads = (
        [p.get_payload() for p in msg.get_payload()] if msg.is_multipart() else []
    )
    assert msg.is_multipart()
    assert any(
        "echo original" in p for p in payloads
    )  # operator part preserved as its own part
    assert any(
        "shutdown -h +10" in p for p in payloads
    )  # TTL appended alongside, not nesting it


def test_ec2_self_terminate_can_be_disabled():
    cfg = Ec2Config(
        region="us-east-1", image_id="ami-x", max_duration_s=1800, self_terminate=False
    )
    fake = FakeAws(
        {**_IDENT, "ec2 run-instances": {"Instances": [{"InstanceId": "i-1"}]}}
    )
...
def _sweep_rt(orphan_max_age_s=3600.0):  # noqa: ANN001
    cfg = Ec2HibernateConfig(
        region="us-east-1", image_id="ami-x", orphan_max_age_s=orphan_max_age_s
    )
    fake = FakeAws(
        {
            **_IDENT,
...
def _stopped_inst(iid, run_tag, when="2026-07-11 00:00:00"):  # noqa: ANN001
    return {
        ...
        "Tags": [
            {"Key": "blastbox-tier", "Value": "aws-ec2-hibernate"},
...
def test_orphan_sweep_terminates_leaked_stopped_slot():
    ...
    killed = rt.sweep_orphans(
        now=_gmt_epoch("2026-07-11 00:00:00") + 7200
    )  # 2h old > 1h max
    assert killed == ["i-old"]
...
def test_orphan_sweep_skips_too_young():
    ...
    killed = rt.sweep_orphans(
        now=_gmt_epoch("2026-07-11 00:00:00") + 60
    )  # 60s < 1h max
    assert killed == []
...
def test_orphan_sweep_dry_run_lists_without_terminating():
    ...
    killed = rt.sweep_orphans(
        now=_gmt_epoch("2026-07-11 00:00:00") + 7200, dry_run=True
    )
    assert killed == ["i-old"]
    assert not any(k == "ec2 terminate-instances" for k, _ in fake.calls)
...
def test_claim_probe_budget_is_thread_local_not_shared():
    ...
    try:
        ...
        t = threading.Thread(
            target=lambda: (
                rt.is_alive(slot),
                other.extend([to for op, to in seen if op == "get-microvm"][-1:]),
            ),
            daemon=True,
        )
        t.start()
...
def test_claim_probe_budget_is_restored_not_cleared():
    ...
    with rt._claim_probe_budget():
        ...
        assert rt._tls.probe_deadline == outer, (
            "nested scope clobbered the outer deadline"
        )
...
def test_claim_probe_timeout_reports_unknown_not_dead():
    ...
    assert rt.is_alive_for_claim(slot) is None, (
        "a probe timeout must be UNKNOWN, not dead"
    )
    ...
    assert rt.is_alive_for_claim(slot) is None, (
        "a host-side exec failure is UNKNOWN, not death"
    )
    ...
    def _gone(argv, timeout):  # noqa: ANN001
        return _cp(
            rc=254,
            stderr="An error occurred (ResourceNotFoundException): no such microvm",
        )
...
def test_claim_mint_timeout_reports_unknown_not_unusable():
    ...
    assert rt.is_alive_for_claim(slot) is None, (
        "a mint timeout must be UNKNOWN, not unusable"
    )
...
def test_claim_probe_budget_is_shared_across_the_whole_probe():
    ...
    now = {"t": 1000.0}
    cfg = LambdaMicroVmConfig(
        region="us-east-1", image_identifier="arn:img", allow_default_egress=True
    )
    fake = FakeAws(
        {
            **_IDENT,
    ...
    rt = LambdaMicroVmRuntime(
        cfg, aws_runner=fake, http_probe=lambda u, h, t: True, clock=lambda: now["t"]
    )
    slot = rt._launch()
    slot.auth_token = "old"
...
def test_subclass_config_positional_binding_survives_the_new_base_field():
    ...
    cfg = LambdaMicroVmConfig(
        "us-east-1", "prof", 8765, "/hz", 300.0, 5.0, 120.0, 3600.0, "arn:img"
    )
    assert cfg.image_identifier == "arn:img", (
        "the new base field rebound a subclass field"
    )
...
def test_transient_control_plane_answer_is_unknown_not_dead():
    ...
    for stderr in (
        ...
        rt = LambdaMicroVmRuntime(
            LambdaMicroVmConfig(
                region="us-east-1", image_identifier="arn:x", allow_default_egress=True
            ),
            aws_runner=runner,
        )
        ...
        assert rt.is_alive(slot) is None, (
            f"{stderr!r} read as a definitive verdict on the health tick"
        )
        assert rt.is_alive(slot) is not False, (
            f"{stderr!r} read as DEAD on the health tick"
        )
    ...
    gone = lambda argv, timeout: subprocess.CompletedProcess(  # noqa: E731
        ...
        255,
        "",
        "An error occurred (ResourceNotFoundException): microvm not found",
    )
    ...
    assert (
        rt2.is_alive_for_claim(AwsWorkerSlot(slot_id="s2", resource_id="mv-2")) is False
    )
...
def test_every_tier_clamps_non_positive_probe_budgets():
    ...
    for cls, kw in (
        (LambdaMicroVmConfig, {"image_identifier": "i"}),
        (LambdaSnapStartConfig, {"image_identifier": "i"}),
        (Ec2Config, {"image_id": "ami-0"}),
    ):
        cfg = cls(
            region="us-east-1", claim_probe_timeout_s=0, health_probe_timeout_s=-5, **kw
        )
        assert cfg.claim_probe_timeout_s > 0, (
            f"{cls.__name__} would brick: claim budget 0"
        )
        assert cfg.health_probe_timeout_s > 0, (
            f"{cls.__name__} health budget non-positive"
        )
...
def test_f1_transient_rc_outside_a_probe_budget_is_unknown_not_dead():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": _cp(
                rc=255, stderr="ThrottlingException: Rate exceeded"
            )
        }
    )
    with pytest.raises(AwsWorkerError) as ei:
...
def test_f1_a_confirmed_dead_verdict_is_still_dead():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": _cp(
                rc=254, stderr="ResourceNotFoundException: no such microvm"
            )
        }
    )
    with pytest.raises(AwsWorkerError) as ei:
...
def test_f4_health_path_token_mint_honours_the_health_budget():
    ...
    cfg = LambdaMicroVmConfig(
        region="us-east-1",
        image_identifier="arn:aws:lambda:us-east-1:aws:microvm-image:x",
        allow_default_egress=True,
    ...
    rt = LambdaMicroVmRuntime(
        cfg, aws_runner=fake, http_probe=lambda u, h, t: True, clock=lambda: 100.0
    )
    slot = AwsWorkerSlot(
        slot_id="s1",
...
def test_f5_claim_probe_never_outruns_the_callers_claim_deadline():
    ...
    fake = RecordingAws(
        {**_IDENT, "lambda-microvms get-microvm": {"Microvm": {"State": "RUNNING"}}}
    )
...
def test_f8_a_resume_that_exhausts_its_deadline_on_timeouts_is_still_unknown():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": _timeout,
    ...
    assert isinstance(ei.value.__cause__, AwsUnknownState), (
        "the originating cause was not chained"
    )
...
def test_f9_a_definitive_error_carrying_503_digits_is_not_transient(stderr):
    ...
    assert _is_confirmed_dead_aws_error(stderr), (
        "a definitive NotFound must be read as dead"
    )
...
def test_f9_a_real_503_is_still_transient():
    ...
    assert not _is_confirmed_dead_aws_error(
        "An error occurred (503) when calling the GetMicrovm operation"
    )
...
def test_f15_an_unrecognised_cli_failure_raises_unknown_not_error():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": _cp(
                rc=255, stderr="An error occurred (TooManyRequestsException)"
            )
        }
    )
...
def test_f14_a_throttled_token_mint_does_not_kill_the_worker():
    ...
    with pytest.raises(AwsWorkerError) as ei:
        rt.resume(slot)
    assert _is_unknown_not_dead(ei.value), (
        "a throttled mint is not evidence the worker is dead"
    )
...
def test_f19_a_recovered_control_plane_clears_a_stale_unknown():
    ...
    def flaky_describe(argv):  # noqa: ANN001
        ...
        return _cp(
            stdout=json.dumps({"state": "RUNNING", "endpoint": "vm.example"})
        )  # ... then fine

    tick = [100.0]
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": flaky_describe,
            "lambda-microvms resume-microvm": {},
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
        },
...
def test_f21_resume_honours_a_shorter_claim_budget():
    ...
    spent = tick[0] - start
    assert spent < 50.0, (
        f"resume ignored the 0.5s claim budget and ran for {spent} clock-seconds"
    )
...
def test_f14_health_ok_propagates_an_unknown_mint_rather_than_returning_false():
    ...
    with pytest.raises(AwsUnknownState):
        rt._health_ok(
            slot
        )  # first call: nothing suppressed yet, so this is the raise site
...
def test_f23_a_resume_that_never_got_to_try_is_unknown_not_failure():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": {"state": "SUSPENDED", "endpoint": "vm.x"},
            "lambda-microvms resume-microvm": {},
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
        },
        probe=lambda u, h, t: True,
    ...
    assert _is_unknown_not_dead(ei.value), (
        "a slot we never got to probe must never be destroyed"
    )
    ...
    rt, _ = _snapstart_rt(
        {
            ...
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
            "lambda-microvms resume-microvm": _cp(
                rc=255, stderr="An error occurred (ConflictException): not SUSPENDED"
            ),
        },
    ...
    rt, _ = _snapstart_rt(
        {
            ...
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
            "lambda-microvms resume-microvm": _cp(
                rc=255, stderr="An error occurred (ThrottlingException): slow down"
            ),
        },
        probe=lambda u, h, t: False,
        clock=lambda: tick.__setitem__(0, tick[0] + 0.05) or tick[0],
...
def test_credential_errors_are_never_confirmed_worker_death():
    ...
    assert not dead(
        "An error occurred (InvalidAccessKeyId) when calling DescribeInstances: "
        "The AWS Access Key Id you provided does not exist in our records."
    )
    assert not dead(
        "An error occurred (AuthFailure): AWS was not able to validate the credentials"
    )
    assert not dead(
...
def test_a_truncated_positive_claim_budget_is_unknown_not_failure():
    ...
    with pytest.raises(AwsUnknownState) as ei:
        rt.resume(
            slot, budget_s=0.2
        )  # positive, but a fraction of the real resume budget
    assert _is_unknown_not_dead(ei.value), (
        "a window we truncated ourselves is not a worker verdict"
    )
...
def test_a_fair_budget_still_bounds_the_calls_inside_resume():
    ...
    assert not over, (
        f"calls exceeded the 10s claim budget and ran at cli_timeout_s: {over[:3]}"
    )
...
def test_hibernate_resume_does_not_grant_a_second_full_budget():
    ...
    def runner(argv, timeout):  # noqa: ANN001
        ...
        if op == "sts get-caller-identity":
            return _cp(
                stdout=json.dumps({"Account": "1", "Arn": "arn:aws:iam::1:user/x"})
            )
    ...
    slot = AwsWorkerSlot(
        slot_id="h1", resource_id="i-1", state=SlotState.ASSIGNED, ip="10.0.0.5"
    )
    start = tick[0]
    with pytest.raises(AwsWorkerError):
...
def test_hibernate_rejects_a_zero_claim_budget_before_touching_the_instance():
    ...
    rt = Ec2HibernateRuntime(
        Ec2HibernateConfig(region="us-east-1", image_id="ami-0abc"),
        aws_runner=runner,
        http_probe=lambda u, h, to: False,
        clock=lambda: 100.0,
    )
    slot = AwsWorkerSlot(
        slot_id="h1", resource_id="i-1", state=SlotState.ASSIGNED, ip="10.0.0.5"
    ...
    with pytest.raises(AwsUnknownState) as ei:
        rt.resume(slot, budget_s=0.0)
    assert "ec2 start-instances" not in ops, (
        f"the instance was started with no budget to wake it: {ops}"
    )
...
def test_an_exhausted_window_skips_the_probe_instead_of_issuing_a_zero_timeout():
    ...
    slot = AwsWorkerSlot(
        slot_id="p1", resource_id="i-1", state=SlotState.ASSIGNED, ip="10.0.0.5"
    )
    # pin the thread-local budget to "already exhausted"
    rt._tls.probe_deadline = rt._clock()
    try:
        assert rt._health_ok(slot) is None, (
            "an unaskable probe must report UNKNOWN, not not-healthy"
        )
...
def test_a_small_configured_probe_timeout_does_not_brick_the_tier():
    ...
    assert cfg.probe_timeout_s >= _MIN_PROBE_S, (
        "a sub-floor probe timeout bricks the tier"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            errno.EADDRNOTAVAIL,
            True,
        ),  # ephemeral ports exhausted -- purely OUR side,
        (
            "ENETUNREACH",
            errno.ENETUNREACH,
            True,
        ),  # and both hit every worker on the same tick
...
def test_local_errnos_are_separated_from_real_verdicts(name, code, is_local):
    ...
    wrapped = urllib.error.URLError(
        OSError(code, name)
    )  # the shape urllib actually raises
...
def test_dns_failures_are_unknown_not_a_worker_verdict():
    ...
    wrapped = urllib.error.URLError(
        socket.gaierror(socket.EAI_AGAIN, "[EAI_AGAIN] Temporary failure")
    )
    assert _is_local_resource_error(wrapped), (
        "a temporary resolver failure is not a worker verdict"
    )
    ...
    nxdomain = urllib.error.URLError(
        socket.gaierror(socket.EAI_NONAME, "[EAI_NONAME] Name unknown")
    )
...
def test_an_unparseable_aws_response_is_unknown_not_death():
    ...
    rt, _ = _snapstart_rt(
        {"lambda-microvms get-microvm": _cp(stdout="<html>502 Bad Gateway</html>")}
    )
    with pytest.raises(AwsUnknownState):
        rt._aws("lambda-microvms", "get-microvm", "--microvm-identifier", "mv-1")
...
def test_a_confirmed_dead_microvm_is_retired_not_handed_back():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": _cp(
                rc=254, stderr="An error occurred (ResourceNotFoundException)"
            ),
            "lambda-microvms resume-microvm": {},
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
        },
...
def test_a_confirmed_dead_verdict_survives_a_later_budget_timeout():
    ...
    def flaky(argv):  # noqa: ANN001
        ...
        raise subprocess.TimeoutExpired(
            cmd="aws", timeout=120
        )  # ...then everything times out

    tick = [100.0]
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": flaky,
...
def test_a_slot_that_suspends_between_passes_is_not_convicted():
    ...
    def describe(argv):  # noqa: ANN001
        return _cp(
            stdout=json.dumps({"state": next(states, "SUSPENDED"), "endpoint": "vm.x"})
        )

    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": describe,
            "lambda-microvms resume-microvm": {},
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
        },
...
def test_an_unaddressable_instance_is_unknown_not_silent():
    ...
    assert rt._agent_healthy(slot) is None, (
        "no address yet was reported as the agent being silent"
    )
...
def test_a_mint_unknown_reaches_the_pool_from_the_HEALTH_path_too():
    ...
    cfg = LambdaMicroVmConfig(
        region="us-east-1",
        image_identifier="arn:aws:lambda:us-east-1:aws:x",
        allow_default_egress=True,
    )
    fake = FakeAws(
        {
            **_IDENT,
            "lambda-microvms get-microvm": {"state": "RUNNING"},
            "lambda-microvms create-microvm-auth-token": _cp(
                rc=255, stderr="An error occurred (ThrottlingException): Rate exceeded"
            ),
        }
    )
    rt = LambdaMicroVmRuntime(
    ...
    slot.token_minted_at = (
        -1e9
    )  # far past half-TTL -> forces a re-mint, which is throttled
...
def test_snapstart_unresolved_endpoint_is_unknown_with_zero_probes():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": {"state": "RUNNING"},  # no endpoint yet
            "lambda-microvms resume-microvm": {},
            "lambda-microvms create-microvm-auth-token": {"authToken": "jwe"},
        },
        probe=probe,
    ...
    with pytest.raises(AwsUnknownState):
        rt.resume(slot)
    assert probes[0] == 0, (
        f"convicted after issuing {probes[0]} probes — it issued none"
    )
...
def test_a_confirmed_dead_mint_error_is_not_swallowed_into_a_backoff():
    ...
    rt, _ = _snapstart_rt(
        {
            "lambda-microvms get-microvm": _cp(
                rc=255, stderr="An error occurred (ThrottlingException)"
            ),
            "lambda-microvms resume-microvm": _cp(
                rc=255, stderr="An error occurred (ThrottlingException)"
            ),
...
def test_is_ready_reports_unknown_when_the_control_plane_throttles():
    ...
    slot = rt.spawn()
    assert rt.is_ready(slot) is None, (
        "a throttled describe must be UNKNOWN, not not-ready"
    )
...
def test_a_throttle_leaves_the_hibernate_phase_untouched():
    ...
    assert rt._phase.get(slot.slot_id) == phase_before, (
        "a brownout must not rewind the phase"
    )
...
def test_available_reports_false_when_the_probe_fails_definitively():
    ...
    def denied(argv):  # noqa: ANN001
        raise AwsWorkerError(
            "AccessDenied: not authorized to perform sts:GetCallerIdentity"
        )
...
def test_a_throttled_call_raises_the_retryable_type():
    ...
    def throttled(argv):  # noqa: ANN001
        return _cp(
            rc=254, stderr="An error occurred (ThrottlingException): Rate exceeded"
        )
...
def test_an_access_denied_is_a_verdict_not_a_retry():
    ...
    def denied(argv):  # noqa: ANN001
        return _cp(
            rc=254,
            stderr="An error occurred (AccessDenied) when calling GetCallerIdentity",
        )
...
def test_a_half_succeeded_resume_is_re_parked(tmp_path=None):
    ...
    assert rt._phase["s"] == "warming", (
        "the mismatch must be noticed from the OBSERVED state"
    )
...
def test_a_hibernation_stuck_in_stopping_is_eventually_given_up_on():
    ...
    state, healthy, ticks = ["stopping"], [True], [1000.0]
    rt, _ = _hibernate_rt(
        state=state, healthy=healthy, clock=lambda: ticks[0], hibernate_timeout_s=300.0
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="i-1")
    ...
    assert rt.maintain_idle(slot) is False, (
        "a stuck hibernation must be given up on, not held forever"
    )
...
def test_maintain_idle_changes_nothing_when_the_control_plane_is_silent():
    ...
    cfg = Ec2HibernateConfig(region="us-east-1", image_id="ami-x", resume_poll_s=0.0)
    rt = Ec2HibernateRuntime(
        cfg, aws_runner=fake, http_probe=lambda u, h, t: True, clock=lambda: 1000.0
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="i-1")
    rt._phase["s"] = "parked"

    assert rt.maintain_idle(slot) is True, (
        "a brownout must not retire a healthy parked slot"
    )
...
def test_a_definitive_park_refusal_is_not_frozen_like_a_brownout():
    ...
    state, healthy, ticks = ["running"], [True], [1000.0]
    rt, fake = _hibernate_rt(
        state=state, healthy=healthy, clock=lambda: ticks[0], hibernate_timeout_s=120.0
    )
    fake.responses["ec2 stop-instances"] = lambda argv: _cp(
        rc=254,
        stderr="An error occurred (AccessDenied) when calling the StopInstances operation",
    )
...
def test_the_second_no_verdict_door_in_maintenance_freezes_too():
    ...
    assert rt.maintain_idle(slot) is True, (
        "a no-verdict must not report the slot unusable"
    )
...
def test_the_maintenance_door_freezes_the_park_clock_too():
    ...
    fake.responses["ec2 describe-instances"] = lambda argv: _cp(
        rc=254,
        stderr="An error occurred (RequestLimitExceeded) when calling DescribeInstances",
    )
...
def test_the_settle_window_survives_a_slow_stop():
    ...
    fake.responses["ec2 stop-instances"] = slow_stop
    slot = AwsWorkerSlot(
        slot_id="s", resource_id="i-1", state=SlotState.IDLE, url="http://10.0.0.5:8080"
    )
...
def test_the_boot_warm_up_is_not_credited_to_the_park_clock():
    ...
    rt._freeze_park(
        slot.slot_id, ticks[0]
    )  # agent unprobeable at boot (NOT a park attempt)
    ticks[0] += 120.0
...
def test_a_lost_response_hibernate_is_still_recognised_as_parked():
    ...
    rt._freeze_park(
        slot.slot_id, 1000.0, park_attempted=True
    )  # stop issued, no verdict
...
def test_a_write_racing_a_reap_cannot_resurrect_a_departed_slot():
    ...
    fake.responses[
        "ec2 terminate-instances"
    ] = {}  # the reap must SUCCEED for a tombstone
    ...
    leaked = [
        name
        for name, d in (
            ("_park_unknown_since", rt._park_unknown_since),
...
def test_a_no_verdict_stop_is_unknown_to_the_pool_too():
    ...
    fake.responses["ec2 stop-instances"] = lambda argv: _cp(
        rc=254,
        stderr="An error occurred (RequestLimitExceeded) when calling StopInstances",
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="i-1")
...
def test_a_stale_running_read_does_not_erase_the_hibernation_claim_guard():
    ...
    assert rt.is_alive_for_claim(slot, budget_s=5.0) is None, (
        "the slot must stay unclaimable"
    )
...
def test_a_failed_terminate_keeps_the_evidence_needed_to_reclaim_the_slot():
    ...
    fake.responses["ec2 terminate-instances"] = lambda argv: _cp(
        rc=254,
        stderr="An error occurred (RequestLimitExceeded) when calling TerminateInstances",
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="i-1")
...
def test_crediting_never_puts_the_park_clock_in_the_future():
    ...
    rt._freeze_park(
        slot.slot_id, 1000.0, park_attempted=True
    )  # no verdict, and NO _park_since
    assert slot.slot_id not in rt._park_since, (
        "the lost-response case starts with no clock"
    )
...
def test_an_unresolved_park_attempt_makes_the_slot_unclaimable():
    ...
    assert rt._phase.get(slot.slot_id) != "hibernating", (
        "the lost-response case: phase unchanged"
    )
...
def test_a_server_side_timeout_is_a_non_answer_not_a_refusal():
    ...
    for stderr in (
        ...
        assert _is_throttle_aws_error(stderr), (
            f"classified as a definitive answer: {stderr[:60]}"
        )
...
def test_a_stop_api_brownout_does_not_drain_the_hibernate_tier():
    ...
    for _ in range(14):  # 14 x 30s = 420s > 300s of stop-API brownout
        assert (
            rt.maintain_idle(slot := AwsWorkerSlot(slot_id="s", resource_id="i-1"))
            is True
...
def test_a_slow_failed_describe_memo_outlives_the_call_that_wrote_it():
    ...
    rt, _ = _snapstart_rt(
        {"lambda-microvms get-microvm": slow_fail}, clock=lambda: ticks[0]
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="mv-1")
    assert rt.is_ready(slot) is None
    assert n["describe"] == 1, (
...
def test_a_refusal_followed_by_an_acceptance_clears_the_mark():
    ...
    rt.is_ready(slot)
    assert "s" not in rt._park_refused, (
        "an accepted attempt must clear the refusal mark"
    )
...
def test_an_empty_service_probe_is_no_verdict_not_availability():
    ...
    rt, _ = _snapstart_rt(
        {**_IDENT, "lambda-microvms list-microvms": _cp(stdout="   ")}
    )
...
def test_an_empty_instance_type_probe_does_not_condemn_the_tier():
    ...
    with pytest.raises(AwsNoVerdict) as ei:
        rt._service_available()
    assert not (
        isinstance(ei.value, AwsWorkerError)
...
def test_a_real_unsupported_instance_type_is_still_a_verdict():
    ...
    fake.responses["ec2 describe-instance-types"] = {
        "InstanceTypes": [
            {"HibernationSupported": False, "MemoryInfo": {"SizeInMiB": 512}}
        ]
    }
...
def test_a_newer_attempt_supersedes_an_older_refusal():
    ...
    rt.is_ready(slot)
    assert "s" not in rt._park_refused, (
        "the stale refusal outlived the attempt it described"
    )
...
def test_an_operator_stop_is_not_adopted_as_our_hibernation():
    ...
    assert "s" not in rt._park_since, (
        "the observation started a park episode we never asked for"
    )

    now[0] += 5.0
    state[0] = "stopped"
    assert rt.is_ready(slot) is False, (
        "adopted an operator-stopped instance as a parked warm slot"
    )
    assert rt._phase.get("s") != "parked"
...
def test_our_own_stop_is_still_adopted_from_stopping():
    ...
    assert rt._phase.get("s") == "hibernating", (
        "our own in-flight hibernation was not adopted"
    )
    now[0] += 5.0
    state[0] = "stopped"
    assert rt.is_ready(slot) is True
...
def test_a_parked_slot_seen_stopping_is_not_our_hibernation_either():
    ...
    assert rt.is_ready(slot) is False
    assert "s" not in rt._hib_started, (
        "a parked phase manufactured fresh in-flight parking evidence"
    )
    assert "s" not in rt._park_since
...
def test_a_stop_that_never_left_the_host_is_not_a_hibernation_attempt():
    ...
    assert "s" in rt._park_unknown_since, (
        "the give-up clock must still freeze -- we learned nothing"
    )
...
def test_a_stop_that_was_issued_and_went_unanswered_IS_still_an_attempt():
    ...
    assert "s" in rt._park_attempted, (
        "a genuine unresolved stop must stay recorded as an attempt"
    )
...
def test_a_blank_token_response_is_no_answer_rather_than_a_dead_worker():
    ...
    rt, fake = _lambda_rt(
        {"lambda-microvms create-microvm-auth-token": lambda argv: _cp(stdout="")}
    )
    with pytest.raises(AwsNoVerdict):
...
def test_a_clock_skew_rejection_is_not_a_hibernation_attempt():
    ...
    fake.responses["ec2 stop-instances"] = lambda argv: _cp(
        rc=254,
        stderr="An error occurred (RequestTimeTooSkewed) when calling StopInstances",
    )
    rt._phase["s"] = "hibernating"
    rt.is_ready(slot)
    now[0] += 5.0
    ...
    assert "s" in rt._park_unknown_since, (
        "we still learned nothing, so the clock must freeze"
    )
...
def test_a_clock_skew_rejection_still_leaves_tier_availability_undecided():
    ...
    for k in list(fake.responses):
        fake.responses[k] = lambda argv: _cp(
            rc=254,
            stderr="An error occurred (RequestTimeTooSkewed) when calling GetCallerIdentity",
        )
    with pytest.raises(AwsNoVerdict) as ei:
        rt.available()
    assert _is_undecided_availability(ei.value), (
        "clock skew must stay deferrable, not drop the tier"
    )
...
def test_a_refused_stop_for_a_reaped_slot_recreates_no_state():
    ...
    def _refused_then_reaped(argv):  # noqa: ANN001 -- the slot is reaped DURING the call
        ...
        return _cp(
            rc=254,
            stderr="An error occurred (InvalidParameterValue) calling StopInstances",
        )

    fake.responses[
        "ec2 terminate-instances"
    ] = {}  # the reap must SUCCEED to leave a tombstone
    fake.responses["ec2 stop-instances"] = _refused_then_reaped
    assert rt._try_park(slot) is None, (
        "a reaped slot must not report an answered refusal"
    )
    assert "s" not in rt._park_refused
    assert "s" not in rt._park_since
...
def test_a_resuming_slot_is_not_evidence_of_a_hibernation_we_performed():
    ...
    assert rt.is_ready(slot) is False, (
        "a resuming slot found stopped was adopted as parked"
    )
    assert rt._phase.get("s") != "parked"
...
def test_an_outage_before_recovery_still_goes_through_the_credit_cap():
    ...
    state = ["running"]
    rt, _ = _hibernate_rt(
        state=state, healthy=[True], clock=lambda: now[0], hibernate_timeout_s=300.0
    )
    slot = AwsWorkerSlot(slot_id="s", resource_id="i-1")

    rt._freeze_park(
        "s", now[0], park_attempted=True
    )  # we issued it; no verdict came back
    ...
    fired = next(
        (
            t
            for t in range(1000, 40001, 10)
            if (now.__setitem__(0, float(t)) or rt._park_expired("s", float(t)))
        ),
        None,
    )
...
def test_the_maintenance_budget_expiring_is_UNKNOWN_not_a_verdict():
    ...
    def _too_slow(argv):  # noqa: ANN001 -- the control plane outruns the pool's tick budget
        raise AwsProbeTimeout(
            "aws ec2 describe-instances: exceeded the maintenance budget"
        )
...
def test_sibling_hibernate_tiers_do_not_sweep_each_others_live_parked_slots():
    ...
    assert not swept_ours, (
        f"the sweep terminated this process's own parked slot: {killed}"
    )
...
def test_a_throttle_still_leaves_tier_availability_undecided():
    ...
    for k in list(fake.responses):
        fake.responses[k] = lambda argv: _cp(
            rc=254,
            stderr="An error occurred (ThrottlingException) when calling GetCallerIdentity",
        )
    with pytest.raises(AwsNoVerdict) as ei:
        rt.available()
...
def test_a_rate_limit_is_both_retryable_and_never_performed():
    ...
    with pytest.raises(AwsRateLimited) as ei:
        rt._describe(AwsWorkerSlot(slot_id="s", resource_id="i-1"))
    assert isinstance(ei.value, AwsThrottled), (
        "a rate limit must stay the retryable type"
    )
    assert isinstance(ei.value, AwsNotExecuted), (
        "a rate limit was refused, not performed"
    )
...
def test_a_reap_landing_mid_park_step_cannot_be_outrun_by_the_step():
    ...
    assert reaper_done.wait(timeout=5.0), (
        "the reaper thread never finished; the lock is not being released"
    )
    ...
    assert "s" not in rt._park_attempted, (
        "_park_attempted was re-created for a reaped slot"
    )
...
def test_every_park_state_write_goes_through_the_lock():
    ...
    cls = next(
        n
        for n in tree.body
        if isinstance(n, ast.ClassDef) and n.name == "Ec2HibernateRuntime"
    )
    ...
    for node in ast.walk(cls):
        ...
        if target and not _locked_context(node):
            unguarded.append(
                f"{_fn_of(node)}() writes self.{target} at line {node.lineno}"
            )
    ...
    for node in ast.walk(cls):
        if (
            ...
            and not _locked_context(node)
        ):
            bad_calls.append(
                f"{_fn_of(node)}() calls {node.func.attr}() at line {node.lineno}"
            )
...
def test_the_health_probe_ceiling_is_configurable_and_cannot_be_set_to_nothing():
    ...
    for cfg_cls in (Ec2Config, Ec2HibernateConfig):
        ...
        for bad in ("nan", "inf", "0", "-5"):
            env = {
                "BLASTBOX_EC2_AMI": "ami-x",
                "BLASTBOX_AWS_HEALTH_PROBE_TIMEOUT_S": bad,
            }
...
def test_a_describe_landing_after_reap_leaves_no_cache_entry_behind():
    ...
    assert "s" not in rt._desc_fail_at, (
        "_desc_fail_at leaked an entry for a reaped slot"
    )
...
def test_the_hibernate_tier_rejects_a_timeout_that_cannot_mean_a_duration():
    ...
    assert (
        Ec2HibernateConfig.from_env(
            {
                "BLASTBOX_EC2_AMI": "ami-x",
                "BLASTBOX_AWS_HEALTH_PROBE_TIMEOUT_S": "nan",
            }.get
        ).health_probe_timeout_s
        == 30.0
    ), "subclass __post_init__ silently replaced the base's clamp"
...
def test_the_park_clock_is_anchored_when_the_calls_finished_not_when_the_pass_started():
    ...
    assert started is not None, (
        "sanity: a refused attempt should still open the give-up clock"
    )
    lag = clock[0] - started
    assert lag < 1.0, (
...
def test_every_float_knob_on_every_aws_tier_rejects_a_non_duration():
    ...
    tiers = [
        obj
        for obj in vars(_aw).values()
        if inspect.isclass(obj)
        and dataclasses.is_dataclass(obj)
        and issubclass(obj, _aw.AwsWorkerConfig)
    ]
    assert len(tiers) >= 4, (
        f"sanity: expected every AWS tier config, found {len(tiers)}"
    )
    ...
    for cls in tiers:
        required = {
            f.name: ("ami-x" if "image_id" in f.name else "x")
        ...
        for field in dataclasses.fields(cls):
            ...
            for bad in bad_values:
                ...
                if (
                    ...
                    or got < 0
                    or (got == 0 and field.name not in allow_zero)
                ):
                    offenders.append(
                        f"{cls.__name__}.{field.name} kept {bad!r} as {got!r}"
                    )
            # ...and a legal value must still survive the clamp
            kept = getattr(cls(**{**required, field.name: 7.0}), field.name)
            if (
                kept != 7.0 and field.name != "probe_timeout_s"
            ):  # probe has a documented floor
                offenders.append(
                    f"{cls.__name__}.{field.name} clobbered a legal 7.0 -> {kept!r}"
                )
...
def test_cache_publication_holds_the_lock_it_checks_the_tombstone_under():
    ...
    def _release_the_reaper() -> None:
        ...
        released.set()
        threading.Thread(
            target=lambda: (rt.reap(slot), done.set()), daemon=True
        ).start()
    ...
    assert done.wait(timeout=5.0), (
        "the reaper never finished; the lock is not being released"
    )
