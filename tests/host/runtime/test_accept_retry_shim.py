...
def test_shim_flips_eintr_bail_to_retry(tmp_path):
    ...
    base = subprocess.run(
        [str(probe), str(tmp_path / "a.sock")], capture_output=True, text=True, timeout=20
    )
    assert base.returncode == 1 and "bailed" in base.stdout, (
        base.returncode, base.stdout, base.stderr,
    )
    ...
    assert fixed.returncode == 0 and "retried" in fixed.stdout, (
        fixed.returncode, fixed.stdout, fixed.stderr,
    )
