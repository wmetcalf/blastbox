...
def test_proxy_url_no_auth():
    assert (
        socks_proxy_url("172.30.0.10:1080", user=None, password=None)
...
def test_proxy_url_with_auth():
    assert (
        socks_proxy_url("10.0.0.5:1080", user="bb", password="bb")
        == "socks5://bb:bb@10.0.0.5:1080"
    )


@pytest.mark.parametrize(
    "bad", ["", "noport", "h:p:x", "host:notaport", "host:99999", " h:1"]
)
...
def test_validate_socks_url_no_auth_roundtrips():
    assert (
        validate_socks_url("socks5://172.30.0.40:9050") == "socks5://172.30.0.40:9050"
    )


def test_validate_socks_url_with_auth_roundtrips():
    assert (
        validate_socks_url("socks5://bb:bb@10.0.0.5:1080")
        == "socks5://bb:bb@10.0.0.5:1080"
    )
...
def test_transproxy_rules_redirect_tcp_and_dns_keyed_on_worker_ip():
    rules = transproxy_redirect_rules(
        "172.30.0.7", trans_port=9040, dns_port=5353, add=True
    )
    ...
    assert (
        rules[1][:6] == ["iptables", "-t", "nat", "-A", "PREROUTING", "-s"]
        and "--dport" in rules[1]
    )
...
def test_transproxy_rules_teardown_is_symmetric_delete():
    add = transproxy_redirect_rules(
        "10.0.0.5", trans_port=9040, dns_port=5353, add=True
    )
    rm = transproxy_redirect_rules(
        "10.0.0.5", trans_port=9040, dns_port=5353, add=False
    )
...
def test_leakguard_from_inspect_modes():
    ...
    assert leakguard_from_inspect(_lg("allip", 7)) == (
        7,
        False,
        False,
    )  # all-IP tier: keep non-TCP


def test_leakguard_from_inspect_none_without_label_or_pid():
    assert (
        leakguard_from_inspect({"Config": {"Labels": {}}, "State": {"Pid": 5}}) is None
    )
...
def test_leak_guard_rules_block_internal_drops_rfc1918_before_accepts():
    rules = leak_guard_rules(
        allow_udp_dns=True, allowed_ports=(80, 443), block_internal=True
    )
    ...
    first_accept = next(
        i for i, r in enumerate(rules) if "ACCEPT" in r and "lo" not in r
    )
...
def test_egress_filter_from_inspect_reads_labels():
    assert egress_filter_from_inspect(
        _egress_inspect(ports="53,80,443", block="1")
    ) == ((53, 80, 443), True)
...
def test_leak_guard_rules_web_only_dns_only_when_53_listed():
    ...
    assert any(
        "udp" in r
        for r in leak_guard_rules(allow_udp_dns=True, allowed_ports=(53, 80, 443))
    )
    assert not any(
        "udp" in r
        for r in leak_guard_rules(allow_udp_dns=True, allowed_ports=(80, 443))
    )
...
def test_leak_guard_rules_block_internal_only_allip_preserves_non_tcp():
    ...
    rules = leak_guard_rules(
        allow_udp_dns=False, block_internal=True, drop_non_tcp=False
    )
...
def test_egress_filter_from_inspect_tolerates_none_label_values():
    ...
    inspect = {
        "Config": {
            "Labels": {
                EGRESS_PORTS_LABEL: None,
                BLOCK_INTERNAL_LABEL: None,
                "blastbox.job_id": "J",
            }
        }
    }
